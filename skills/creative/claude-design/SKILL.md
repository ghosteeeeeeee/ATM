...
## Sketch Mode — 2-3 Variants to Compare

A sub-mode of claude-design for **exploration before commitment**. When the user says "sketch this", "show me what X could look like", "compare layout A vs B", "give me 2-3 takes on this UI", "let me see some variants", "mockup this before I build" — produce 2-3 disposable HTML variants so the user can pick a direction.

### When NOT to Use Sketch Mode

- The user already knows what they want and wants the polished single output → regular claude-design
- The work is production path → use claude-design with `production_path: true` mindset
- The decision is knowable from prior research → skip the variants

### Variant Workflow

**1. Stance first.** Before writing any HTML, name 2-3 distinct design stances the user might pick between. Each stance is a coherent visual + interaction direction, not a color swap.

Examples of stances:
- `calm-editorial` — generous whitespace, serif headlines, muted palette
- `utilitarian-dense` — monospace, tight grid, high information density
- `playful-split` — bold colors, asymmetric layout, motion-heavy

**2. Variant directory layout.** One subdirectory per variant. Each is a self-contained HTML file with its own README.
...
**3. Make them real HTML.** Each variant:
...
**4. Verify variants visually** using browser tools (not just source inspection):
...
`browser_vision` returns an AI description of what's on the page plus a screenshot — catches layout bugs that pure source inspection misses (font imports that silently failed, flex containers that collapsed).

**5. Default CSS reset + system font stack** for fast starts:
...
**6. Variant README** answers:

```markdown
## Variant: {stance name}

### Design stance
{1-2 sentence description of what this direction bets on}
...
**7. Compare stances side-by-side** at the end. Show a final summary the user can react to:
...
|-----------|----------------|-------------------|---------------|
| Whitespace | generous | minimal | asymmetric |
| Type | serif | mono | display |
| Palette | muted | monochrome | bold |
| Motion | none | micro only | hero |
| Best for | thought-leadership | power users | consumer launch |
```

**8. Pick & iterate.** After the user picks a stance, drop the others (move to `.skipped/` or just delete) and iterate on the winner with the regular claude-design workflow. The variants are disposable — a sketch that takes 2 days to "clean up for production" was a bad sketch.

### Attribution

Adapted from the GSD (Get Shit Done) project's `/gsd-sketch` workflow — MIT © 2025 Lex Christopherson. Install the full GSD system with `npx get-shit-done-cc --hermes --global`.
