...
- **Root Root**: `mark_signal_executed()` in `signal_schema.py` uses atomic UPDATE with `WHERE executed=0`. If decider_run crashes after HL order fills but before `mark_signal_executed()` is called, the trade is live but the signal stays PENDING.
...
### Bug 8: Executor-Guardian Race (2026-05-04 — ZK duplicate position)
- **Symptom**: Guardian closes a token as orphan, but signal_compactor has a fresh EXECUTED signal for the same token. HL ends up with 2 positions, only 1 tracked in signal_outcomes.
- **Root Cause**: Guardian and signal_compactor/executor run on independent cycles. No shared state. Guardian sees HL=5 vs DB=4, identifies ZK as orphan, closes it — not knowing signal_compactor already has a fresh EXECUTED signal in-flight. The fresh signal's HL fill arrives ~37s later, creating a second position.
- **Timeline (ZK)**:
  - `21:29:12` signal 646771 (ZK LONG) → EXECUTED
  - `21:33:23` guardian sync — sees 646771
  - `21:34:11` guardian sync — HL=5, DB=4 → ZK is orphan → close_position_hl(ZK)
  - `21:34:12` ZK #1 closed by guardian (no signal_outcomes recorded — polling returned 0)
  - `21:34:49` signal 646953 (ZK LONG) → EXECUTED → HL fill arrives
  - `21:35:21` guardian sync: HL=4, DB=4 — appears in sync
- **signal_outcomes gap**: `_close_orphan_paper_trade_by_id` polls HL fills after the guardian close. If `hl_exit_px == 0.0` (no fill yet), it returns early with `"No HL fill for {token} trade #{trade_id}, will retry next cycle"` — never writing to signal_outcomes. The orphan close is lost from outcome tracking.
- **Fix chosen**: Option 1 — shared closing marker file. Implementation:

  **`hl-sync-guardian.py`** — `_GUARDIAN_CLOSING_FILE` state machine:
  - `_save_closing_marker(token)` — writes to `guardian-closing-markers.json` BEFORE `market_close()` is called
  - `_clear_closing_marker(token)` — clears after `_close_orphan_paper_trade_by_id` completes
  - Stale marker cleanup at cycle start: if marker exists but token not in `hl_pos`, clear it (HL position is gone, close succeeded — just HL fills hadn't propagated for the DB write)
  - If `market_close` fails, marker stays set (guardian retries next cycle)

  **`decider_run.py`** — `_is_guardian_closing(token)` reads the marker file. Must be wired as execution gate:
  ```python
  if _is_guardian_closing(token):
      log(f'SKIP: {token} — guardian closing in progress')
      mark_signal_executed(token, direction, 'SKIPPED', signal_id=sig_id)
      skipped += 1
      continue
  ```

  **Why marker over Option 2**: `_close_orphan_paper_trade_by_id` returns early when `hl_exit_px=0` — writing `signal_outcomes` immediately (before polling) would record a close with no PnL data and no meaningful exit price. The marker approach is cleaner because guardian writes it immediately (no polling dependency) and executor checks it at execution time.
- **Detection**: `scripts/detect_executor_guardian_race.py` — checks for tokens with 2+ EXECUTED signals in <300s window, signal_outcomes gaps, and tight orphan-signal proximity
