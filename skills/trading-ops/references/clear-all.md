...
# Archive to JSON (2026-05-08: PostgreSQL archive tables dropped)
...
out_path = f'/root/.hermes/archive/trades/trades_archive_{TS}.json'
with open(out_path, 'w') as f:
    json.dump({
        'archived_at': datetime.now().isoformat(),
        'source': 'clear_all_archive',
        'count': len(rows),
...
print(f"Archived {len(rows)} closed trades to {out_path}")
print(f"Deleted {cur.rowcount} trades from DB")
...
| DB `trades` table | Archive closed trades to JSON (`/root/.hermes/archive/trades/`) then DELETE |
...
- Always archive before deleting — never DELETE without archiving (closed trades go to `/root/.hermes/archive/trades/` as JSON)
...
- **ab_results are deleted** — A/B test data is not archived automatically; if needed, export before running clear-all
