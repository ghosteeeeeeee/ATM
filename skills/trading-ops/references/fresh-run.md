...
**Archiving moved to JSON (2026-05-08) — PostgreSQL archive tables no longer used.**
...
with open(out_path, 'w') as f:
    json.dump({
        ...
        'source': 'fresh_run_archive',
        'count': len(rows),
        'columns': list(rows[0].keys()) if rows else [],
        'trades': rows
    }, f, indent=2, default=str)
print(f"[brain] Archived {len(rows)} closed trades -> {out_path}")

cur.execute('DELETE FROM trades WHERE status=%s', ('closed',))
conn.commit()
print(f"[brain] Deleted {cur.rowcount} closed rows from trades table")
