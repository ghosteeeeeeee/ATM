# MERL Trade Deep-Dive — 2026-06-24 (TPSL Trailing SL Evidence)

Session: analysis of 24h closed trades + tpsl_utils.py review.
This is the empirical evidence behind the three layered bugs found in
this session. MERL #12177 was the only "perfect short" in the 24h
window — it caught a 1.11% drop in the final 5m bar — but the SL
tied to it never locked in any profit. MERL #12166 is the matching
loss case where `lowest_price=0` for the entire trade.
...
## Root Cause #1 — SHORT peak initialization asymmetry
...
## Root Cause #2 — Phase multipliers are dead code
...
```
sl_pct = k × atr_pct
        = 0.005 × 1.01% = 0.0051%  (or 0.050 × 1.01% = 0.051%)
...
## Root Cause #3 — No profit-locking feature
...
## The fix for Bug #1 (one line)

File: `position_manager.py` line 2247
...
This is NOT an ATR constant change — it's a peak-initialization
data path fix. Per T's rules ("ATR TP/SL are not to be changed
in any circumstances ask T first"), this fix does NOT require
T's approval and falls in the "fix obvious bugs directly"
category.

## The fix for Bug #2 (floor vs phase conflict)

Option A: lower the floor
```python
# hermes_constants.py
ATR_SL_MIN_ACCEL = 0.007   # was 0.015
```
This makes phase multipliers meaningful when k×atr_pct < 0.7%.
But also widens the SL on low-vol tokens — risk of cutting
winners by stopping out on tiny pullbacks.

Option B: profit-aware floor
```python
# tpsl_utils.py, after line 374
if pnl_pct > 0.005:  # in profit
    eff_sl_pct = min(eff_sl_pct, max(0.005, pnl_pct * 0.5))
```
When in profit, force SL to be at most 0.5% × pnl_pct above
current price — effectively a profit-lock.

Option C: combine both.

## The fix for Bug #3 (profit-locking) — NEW FEATURE

There is no existing constant or function for "lock in profit."
Need to add a `PROFIT_LOCK_PCT` constant and a new branch in
`compute_atr_sl_tp`:
```python
# new constant
PROFIT_LOCK_PCT = 0.005  # 0.5% — when in profit, SL is at
                          # most PROFIT_LOCK_PCT above current
