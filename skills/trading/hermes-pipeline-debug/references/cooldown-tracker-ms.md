...
description: 'CRITICAL cooldown investigation guide for Hermes trading system. Covers the TWO separate cooldown stores, which code paths write to which store, and which functions READ from which store. The most common bug: one store has the cooldown but the reader function checks a DIFFERENT store.'
version: 6.0.0
author: Hermes Agent
created: 2026-04-20
updated: 2026-04-23
...
notes:
  - "BUG FIX (2026-04-28) — cascade_flip rollback protection: cascade_flip._close_paper_position() cooldown call was inside the function's main try/except. If _set_loss_cooldown() threw, the except block called conn.rollback() — UNDOING the already-committed close. Fix: wrap cooldown call in its own nested try/except so rollback only affects the cooldown write, not the trade close. cascade_flip is a standalone module with its own close function — any new close function added here must also add cooldown recording."
  - "BUG FIX (2026-04-28) — decider_run.close_position() reason-string blind spot: Was checking 'loss' in reason string to decide cooldown. But MANUALLY closed trades have reason='MANUAL', 'EXIT', etc. — no 'loss' substring. Tokens that lost on manual/counter-signal close were not blocked. Fix: fetch actual pnl_usdt from DB after UPDATE and check pnl < 0."
  - "BUG FIX (2026-04-28) — sync_open_trades.close_paper_trade_db() missing cooldown: This function (sync_open_trades.py) closes paper orphans via _close_paper_trade_db() but had NO cooldown recording. Added full cooldown helpers (same pattern as guardian) and cooldown call after conn.commit() when pnl_usdt < 0."
  - "BUG FIX (2026-04-28) — brain.close_trade() missing cooldown: brain.py close_trade() is called by hype-sync.py and profit-monster.py. It closed trades but recorded NO cooldown. Added _record_loss_cooldown() call after conn.commit() when hype_pnl_usdt < 0. This was the last major uncooled close path."
...
**DEPRECATED / DEAD CODE** — `_record_trade_outcome()` is defined in hl-sync-guardian.py but never actually called by any code path. The cooldown recording for HL live closes (HL_SL_CLOSED, HL_CLOSED, atr_sl_hit, etc.) is handled inline by the specific close functions themselves (sync_pnl_from_hype cut-loser path, _check_hard_stops, etc.), each of which calls `_record_loss_cooldown()` directly.

**Closes that trigger this (via inline calls):** `HL_SL_CLOSED`, `HL_CLOSED`, `atr_sl_hit`, `histogram_zero_cross`, `profit_monster`, `cascade_flip`
...
**THE BUG (pre-2026-04-28):** Did NOT write any cooldown. Counter-signal closes were invisible to the execution blocker. **FIXED (2026-04-28):** now fetches actual pnl_usdt from DB after UPDATE and writes to BOTH stores if pnl < 0.

**Closes that trigger this:** manual_close, counter-signal closes, kill-switch closes

### Writer 4: `cascade_flip._close_paper_position()` — cascade_flip.py (cascade flip close)
Standalone module with its own close function. **BUG (pre-2026-04-28):** Closed trades without any cooldown call. Only the entry *failure* path at line 463 called `_set_loss_cooldown`. **FIXED (2026-04-28):** cooldown now recorded after successful close, wrapped in nested try/except to prevent rollback.

**Closes that trigger this:** `cascade_flip_{pnl}%`

### Writer 5: `sync_open_trades.close_paper_trade_db()` — sync_open_trades.py (orphan paper close)
**BUG (pre-2026-04-28):** Never recorded cooldown. **FIXED (2026-04-28):** Added cooldown helpers (standalone copy of guardian's helpers) and cooldown call when pnl_usdt < 0.

### Writer 6: `brain.close_trade()` — brain.py (hype-sync and profit-monster close)
**BUG (pre-2026-04-28):** Never recorded cooldown. **FIXED (2026-04-28):** Added cooldown helpers and cooldown call when hype_pnl_usdt < 0.
...
**Variant — Reason-string blind spot (2026-04-28):**
`decider_run.close_position()` checked `'loss' in reason.lower()`. But MANUALLY closed trades have reasons like `'MANUAL'`, `'EXIT'`, `'MANUAL_CLOSE'` — none contain `'loss'`. A trade that lost $2 on a manual close would NOT trigger cooldown because the string `'loss'` didn't appear. Fix: always fetch actual `pnl_usdt` from the DB row after close and check `pnl < 0`.

**Rule:** Never determine loss/win from a reason string. Always compute from actual PnL values.

**Variant — Cooldown inside shared try/except causes rollback (2026-04-28):**
If cooldown recording is placed inside a function's main try/except block (shared with the `conn.commit()` call), an exception in `_set_loss_cooldown()` will trigger `conn.rollback()` — undoing the committed close. Both the close AND the cooldown are lost. Fix: wrap the cooldown call in its own nested try/except, AFTER `conn.commit()` but OUTSIDE the main exception handler.

**Prevention checklist — Before deploying any new close function:**
- [ ] Is the cooldown call AFTER `conn.commit()`, not inside the main try/except?
- [ ] Is loss/win determined by actual PnL value, not by parsing the reason string?
- [ ] Are cooldown helpers (or a reference to them) available in this file?
- [ ] Is the file path importing from paths.py so it uses the same LOSS_COOLDOWN_FILE? (cross-service writes to the same JSON file must use the same path)
- [ ] Is the reader (signal_schema.get_cooldown) checking the store this writer writes to?

**Variant — Standalone module with its own close function (2026-04-28):**
`cascade_flip.py` is a standalone module imported by position_manager. It has its own `_close_paper_position()` function that closes trades with reason=`cascade_flip_{pnl}%`. This function had NO cooldown call — only the entry *failure* path at line 463 called `_set_loss_cooldown`. The losing close itself was silent.
**Fix:** Add cooldown recording in the close function itself, after `conn.commit()`:
```python
if pnl_usdt_val < 0:
    _set_loss_cooldown(token, direction)
```
**Rule:** Any standalone module that closes trades MUST have cooldown recording in its close function. Do not assume the caller will handle it.
...
**Symptom:** Token appears in hot-set every compaction cycle despite being in cooldown. The token survives because it has no fresh signals (score=0) so `_preserve_previous_hotset()` calls `_filter_safe_prev_hotset()` to re-add it — without checking cooldowns.
