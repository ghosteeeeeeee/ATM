...
**Bare names** (`hzscore`, `vel-hermes`, `pct-hermes`) are **blocklisted**. **Suffixed names** (`hzscore+`, `vel-hermes-`) are allowed **only if empirically validated**.

**⚠️ CRITICAL (2026-05-05): `hzscore+` and `hzscore-` directional variants are NOT safe just because bare `hzscore` is blocked.** Empirical data from 128 May 5 trades:
- `hzscore+` (SHORT when price above mean): **38 trades | WR=18% | PnL=-2.16%** — worst signal in the system
- `hzscore-` (LONG when price below mean): **14 trades | WR=43% | PnL=+4.40%** — best directional signal

The directional suffix makes them pass the bare-name blacklist check, but they are **functionally opposite** in bear/choppy markets. `hzscore+` fires SHORT on price near highs — catches falling knives. `hzscore-` fires LONG on price near lows — catches bounces correctly.

**Action required**: Add `hzscore+` and `hzscore-` as explicit component-level blacklist entries in `hermes_constants.py` `SIGNAL_SOURCE_BLACKLIST`. Without this, multi-source combos like `hzscore+,rs-r3111,rs-r3424` pass the component check because only bare `hzscore` is blacklisted, not the suffixed variants.
...
### Pitfall: Stale-DB Bypass
Signals in `signals_hermes_runtime.db` *before* a blacklist entry is added continue to reach trades if the compactor isn't running to expire them. On May 5, `pct-hermes-` (blacklisted Apr 20) still generated 25 trades because old PENDING/APPROVED signals sat unexpired. **Always purge stale signals when adding blacklist entries**:
```bash
python3 -c "
import sqlite3
conn = sqlite3.connect('/root/.hermes/data/signals_hermes_runtime.db')
cur = conn.cursor()
for bl in ['pct-hermes+','pct-hermes-','vel-hermes+','vel-hermes-']:
    cur.execute('UPDATE signals SET decision=\"EXPIRED\" WHERE source LIKE ? AND decision IN (\"PENDING\",\"APPROVED\")', (f'%{bl}%',))
conn.commit()
"
```
Also verify compactor timer: `systemctl list-timers --all | grep signal-compactor`.
