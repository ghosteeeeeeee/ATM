...
5. **ILV signal on dashboard but ILV not on Hyperliquid** — stale `hl_cache.json` mapping (ILV: 12.118 from ~April) polluting token name resolution. Live HL `allMids` returns None for ILV. Signal fired for a ~$4.66 coin but resolved to ILV from stale cache. Guardian cannot execute because token doesn't exist on HL.

## Class 4: Token Not On Exchange — Stale Cache Mapping Pollution

**Symptom**: A coin appears in hot-set, signals.json, or dashboard with a valid signal, but the guardian cannot find it on Hyperliquid to place the order. HL API returns `None` for the token.

**Diagnosis**:
```bash
# Step 1: Verify against live HL API (definitive)
python3 -c "
...
# Step 2: Check stale local cache
python3 -c "
import json
d = json.load(open('/var/www/hermes/data/hl_cache.json'))
mids = d.get('allMids', {})
print('Token in hl_cache.json:', mids.get('TOKEN', 'NOT IN CACHE'))
print('Cache age (minutes):', (json.time.time() - d.get('_ts', 0)) / 60)
"

# Step 3: Check what real coin matches the signal price
python3 -c "
import json
d = json.load(open('/var/www/hermes/data/hl_cache.json'))
mids = d.get('allMids', {})
# Find coins near the signal price
coins_around = {k: float(v) for k, v in mids.items()
                if abs(float(v) - SIGNAL_PRICE) < 1.0 and k.isupper()}
print('Coins near signal price:', coins_around)
"
```

**Root cause chain**:
1. `hl_cache.json` (written by `price_collector.py`) stores `allMids` from HL with a `_ts` timestamp
2. If HL delists a token, the cache still contains the old mapping until overwritten
3. Some pipeline component uses `hl_cache.json` to resolve token names → gets stale mapping
4. Signal generator fires for a coin at price ~$4.66 but token name resolves to ILV from stale cache
5. Guardian receives signal with token=ILV, queries HL live API, gets None → cannot execute

**Key rule**: When a signal's token name doesn't exist on HL but `hl_cache.json` has it — the cache is stale and the token was likely delisted. Always verify against the live API, never the local cache file.

**Fix direction**: `price_collector.py` should validate token existence against live HL data and remove stale entries. Guardian should independently verify token existence against live HL API before order placement, not trust the cached token list.
