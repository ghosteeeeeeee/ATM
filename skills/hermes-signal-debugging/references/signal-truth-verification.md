...
**0. Stale candles.db volume = 0 breaks all volume-dependent signals**
When candles.db has volume=0 for a token's recent candles (common after a data gap), the breakout engine's compression detection is FAKE — volume=0 always passes `vol < 250`. Before trusting any volume-based signal (breakout, vol spike), verify:
```sql
SELECT MAX(ts), COUNT(*), SUM(volume) FROM candles_5m WHERE token='CHIP' AND ts > strftime('%s','now','-2 hours');
-- If SUM(volume)=0 or COUNT(*)=0 for recent bars, volume signals are blind
```
When volume data is stale, fall back to price_history (live) for direction and ATR-normalized range as a volume proxy.

**1. gap300 fires but PERSISTENT_BARS kills V-shape reversals**
gap300_5m requires 3 consecutive 5m bars above MIN_GAP_PCT (0.10%). Sharp V-shape reversals fail this test — the gap crosses positive for 1 bar then pulls back below threshold the next bar. This is the #1 reason gap300 misses reversals. Check PERSISTENT_BARS in gap300_5m_signals.py:32 — consider reducing to 1 for reversals from deeply oversold (gap < -2%).

**2. Don't assume mean-reversion vs momentum**
...
**2. When a breakout fires but gap300 doesn't — check support_resistance**
For V-shape reversals, gap300 often fails due to PERSISTENT_BARS but support_resistance (RS) catches the bounce. If a signal has conf=88 with RS sources (rs-sNN, rs-rNN), check RS levels — they often catch bounces that gap300 misses. Query: `SELECT * FROM signals WHERE token='CHIP' AND created_at > datetime('now','-2 hours') ORDER BY created_at DESC LIMIT 20;`

**3. Off-by-one in PERSISTENT_BARS loop — gap300_5m only checks 2 of 3 bars**
`gap300_5m_signals.py` line ~135: `for k in range(1, PERSISTENT_BARS):` should be `for k in range(1, PERSISTENT_BARS + 1):`. This means the signal fires with weaker persistence than designed — fix is one character.

**4. OC pending entry price is stale — but local price override was added 2026-04-25**
...
**7. candles_1m and candles_5m may be stale while higher TFs are fresh**
