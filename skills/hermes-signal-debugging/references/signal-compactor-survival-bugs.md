...
## Bug 36: RS Signal — Same Single-Source Confluence Gate (REMOVED 2026-04-29)

**Original Symptom**: RS signals emit correctly (85 written to DB, conf=77-88%), but ALL were EXPIRED with `compaction_stale_5min`. Never reached hot-set.

**Original Fix Attempted**: Added RS_SOURCE_PREFIXES bypass in the confluence gate — RS signals would pass without needing a 2nd source.

**REVERTED (2026-04-29)**: T correctly rejected the bypass. RS signals should flow through the same confluence pipeline as all other signals. They naturally merge with other signals (gap-300, vel-hermes, hzscore, etc.) when those fire simultaneously — that merged source is what passes the gate. No special bypass needed.

**Current behavior (2026-04-29)**:
- RS fires as single-source (`rs-s2472`, `rs-r1356`)
- If another signal fires for the same token+direction within ~5 min, they merge into multi-source
- If no merge occurs within 5 min, the RS signal expires (PENDING → EXPIRED)
- This is correct — signals should need confluence like everything else

**HH/HL same pattern**: HH_HL fires as `hhh-short4` / `hhp-long{N}` — single-source by design. When it merged with hzscore+, pct-hermes-, vel-hermes- for AVAX SHORT, it became 5-source and passed the gate correctly. When it doesn't merge, it waits for a 2nd source just like RS.

## Bug 37: HH/HL Signal — Natural Merge Works, No Bypass Needed (2026-04-29)

**Symptom (historical)**: Zero HH/HL signals in hot-set, only 1 ever fired in the DB (AVAX SHORT → merged with hzscore+, pct-hermes-, vel-hermes- → 5-source → passed gate → EXPIRED without execution).

**Analysis (2026-04-29)**: The confluence gate works correctly. HH/HL sources (`hhh-short4`, `hhp-long{N}`) are structurally single-source. When another signal fires for the same token+direction within ~5 min, they merge naturally and pass. AVAX SHORT proved this: `hhh-short4,hzscore+,oc-mtf-macd-,pct-hermes-,vel-hermes-` → 5 sources → passed confluence gate → entered hot-set → expired without execution (decider didn't pick it, not a pipeline failure).

**Conclusion**: No bypass needed. HH/HL waits for a 2nd source naturally, same as RS and all other signals.
...
## Bug 33: Directional Conflict Parser — hzscore+/- Treated as LONG/SHORT Votes (2026-04-29)

**Symptom**: 70+ multi-source PENDING signals (conf=88%, 2+ sources) silently blocked at directional conflict check. Compactor log shows "CONFLICT" skips but the underlying signals had valid multi-source confluence. Empty hot-set despite strong signals in DB.

**Root Cause** (`signal_compactor.py` line ~372): The directional conflict detection loop stripped `+`/`-` suffixes and checked if both polarities existed in the merged source string. But it used this check on ALL sources, including regime/contextual tags that carry `+`/`-` as part of their name (e.g., `hzscore+`, `vel-hermes-`, `gap-300+`, `hzscore-`).

```python
# WRONG — treats hzscore+, gap-300+ as LONG/SHORT polarity votes:
for src in source_parts:
    base = src.replace('+', '').replace('-', '')  # strips suffix
    if base.endswith(('hzscore', 'vel-hermes', 'gap-300', 'fast-momentum')):
        pol = '+' if src.endswith('+') else '-'   # treats hzscore+ as LONG
```

Result: `gap-300+,hzscore-` → conflicts with itself → CONFLICT skip. `gap-300+,vel-hermes-,pct-hermes+` → CONFLICT (vel-hermes- vs gap-300+).

**Fix Applied**: Only these signal sources are actually directional votes (LONG vs SHORT):
```python
DIRECTIONAL_SIGNALS = {'pct-hermes', 'macd-accel', 'ma-cross-5m', 'fast-momentum', 'zscore-momentum'}
```

`hzscore`, `vel-hermes`, `gap-300`, `gap-300-accel` etc. are regime/contextual tags — they describe market condition, not trade direction. The conflict detection now only looks at `DIRECTIONAL_SIGNALS` for polarity conflicts.

**Files changed**: `signal_compactor.py` line ~372.

**Diagnosis**:
```bash
grep "CONFLICT" /root/.hermes/logs/signal-compactor.log | tail -20
```

---

## Bug 34: Redundant hzscore-Only Filter in _filter_safe_prev_hotset (2026-04-29)

**Symptom**: Any preserved entry from `prev_hotset` whose first source was `hzscore` was silently killed, regardless of whether it had 2+ total sources or passed the blacklist check.

**Root Cause** (`signal_compactor.py` line ~1058): A dead-code filter from a prior bugfix era that checked `source_parts[0] == 'hzscore'` and `continue`d — but the same source was already handled by the blacklist check and the 2-source confluence gate upstream.

```python
# Dead code — redundant with blacklist + confluence gate:
if len(source_parts) > 0 and source_parts[0] == 'hzscore':
    log(f"  🔒 [HOTSET-FILTER] {token} {direction}: hzscore-only — skip")
    continue
```

**Fix Applied**: Removed the redundant filter. The blacklist and 2-source confluence gate are the authoritative controls.

**Files changed**: `signal_compactor.py` line ~1058.

---

## Bug 35: oc_pending / oc_rsi — Hard-Block vs Confluence Contribution (REVISED 2026-04-29)

**Original policy**: OC signals were hard-blocked at compactor safety filter regardless of confluence.

**T's correction**: OC signals should COUNT toward the 2+ source confluence requirement (scenario 1). A combo like `pct-hermes+,oc-mtf-macd-` should be allowed — OC provides valid confluence even if the real direction is driven by pct-hermes. Only OC-ONLY combos (no real source at all) should be blocked.

**Symptom that triggered revision**: OC signals were being hard-blocked even when they had real-confluence combos, preventing valid signals from entering the hot-set.

**Revised Fix Applied** (`signal_compactor.py` line ~695): OC signals are NOT hard-blocked. Instead:
- OC-only combo (all sources start with `oc_`, no real signal) → blocked at Step 11
- OC + real-confluence combo (at least one non-OC source) → **allowed through** — OC counts as one of the 2+ sources
...
**Files changed**: `signal_compactor.py` line ~695.

**Key lesson**: External/OC signals should be blocked ONLY when they drive a trade with no other confirmation. When they merge with real signals, they provide valid confluence and should be allowed through the compactor. The test is: does this combo have at least one non-OC source? If yes, let it compete on its merits.
**Key lesson**: OC signal types need blocking at TWO levels: (1) signal_compactor safety filter prevents entry to hot-set; (2) decider_run execution gate is the backup. Both blocks must exist — the compactor block is primary.

---

## Bug 32: Staleness Decay Rate Favors High-Frequency Signals — oc-rsi- / oc-mtf-macd- Cannot Compete (2026-04-29)

**Symptom**: Only gap-300- and pct-hermes+ fire trades. COMP SHORT oc-rsi- (conf=77.3, weight=1.0) and AVGOON SHORT oc-mtf-macd- (conf=88.0, weight=1.0) are never in the hot-set despite scoring reasonably. The two dominant signals (gap-300-, pct-hermes+) both fire 4-5× per minute. oc-rsi- and oc-mtf-macd- fire every few minutes.

**Root Cause — staleness decay rate of 0.2/min kills infrequent signals**:

Formula in `_score_signal()`:
```python
staleness_mult = max(0.0, 1.0 - (age_m * 0.2))  # 0 at 5min
```

This means:
- gap-300- at age 0.5 min → staleness = 0.90 → score = `75 × 0.90 = 67.5`
- oc-rsi- at age 4.2 min → staleness = 0.16 → score = `77 × 0.16 = 12.4`
- gap-300- at age 1.0 min → staleness = 0.80 → still dominates

gap-300- fires every ~15 seconds, so it's always age < 1 min (staleness 0.80+). oc-rsi- fires every few minutes, spending most of its life in the 0.2-0.4 staleness range. A signal with 2× the confidence but 0.2× the frequency cannot compete.

**Evidence from live DB** (2026-04-29):
```
tok=COMP   src=oc-rsi-        dir=SHORT  conf=77.3  age=4.2min  staleness=0.16  eff_score=12.4
tok=AVGOON src=oc-mtf-macd-   dir=SHORT  conf=88.0  age=4.2min  staleness=0.16  eff_score=14.1
tok=AXS    src=gap-300-       dir=LONG    conf=75.0  age=0.5min  staleness=0.90  eff_score=67.5
tok=STBL   src=gap-300-       dir=LONG    conf=75.0  age=0.5min  staleness=0.90  eff_score=67.5
```

COMP/AVGOON conf is comparable or higher than gap-300-, but effective score is 5× lower.

**The weights are NOT the issue**: gap-300- weight = 1.0, oc-rsi- weight = 1.0. Equal weights. The staleness decay is.

**Fix**: Change decay rate from `×0.2` to `×0.08` per minute:
```python
# Before:
staleness_mult = max(0.0, 1.0 - (age_m * 0.2))  # 0 at 5min

# After:
staleness_mult = max(0.0, 1.0 - (age_m * 0.08))  # 0 at 12.5min
```

Also update survival_bonus condition:
```python
# Before:
if compact_rounds > 0 and age_m < 5.0:

# After:
if compact_rounds > 0 and age_m < 12.0:
```

Files: `signal_compactor.py` lines ~205 (staleness_mult formula), ~199 (survival_bonus age_m threshold).

**Key lesson**: The staleness rate defines a signal's effective lifespan. A rate of 0.2/min (5-min life) favors signals that fire every 15 seconds. A rate of 0.08/min (12.5-min life) gives infrequent-but-strong signals like oc-rsi- (88% conf) a fighting chance.
