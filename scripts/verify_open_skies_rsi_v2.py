#!/usr/bin/env python3
"""
Independent verification v2: Compute RSI AT SIGNAL TIME for all open-skies trades.
Uses historical 1h candles around the signal creation timestamp.
"""
...
def get_1h_candles_at_time(token, timestamp, limit=100):
    """Get 1h candles from candles.db BEFORE the given timestamp (historical lookback)."""
    ...
    # Get candles BEFORE the signal time (lookback window)
    cur.execute("""
...
def main():
    print("=" * 80)
    print("INDEPENDENT VERIFICATION v2: Open-Skies RSI Filter Analysis")
    print("Using RSI computed AT SIGNAL TIME from 1h candles")
    print("=" * 80)
    ...
    for trade in trades:
        ...
        # Get 1h candles AT THE TIME OF SIGNAL
        candles = get_1h_candles_at_time(token, created_at, 100)
        
        if not candles or len(candles) < 15:
            skipped += 1
            print(f"⚠️  {token} ({created_at}): Insufficient candle data (only {len(candles)} candles)")
            continue
        ...
        if rsi is None:
            skipped += 1
            print(f"⚠️  {token} ({created_at}): Could not compute RSI")
            continue
    ...
    # Test RSI 60-85 (momentum zone)
    print("\n" + "=" * 80)
    print("TESTING: RSI 60-85 (momentum zone)")
    print("=" * 80)
    
    passed_momentum = [t for t in all_trades if 60 <= t['rsi'] <= 85]
    passed_winners_mom = [t for t in passed_momentum if t['pnl'] > 0]
    passed_losers_mom = [t for t in passed_momentum if t['pnl'] <= 0]
    passed_pnl_mom = sum(t['pnl'] for t in passed_momentum)
    
    print(f"\nTrades passing: {len(passed_momentum)} of {len(all_trades)}")
    print(f"Winners: {len(passed_winners_mom)}")
    print(f"Losers: {len(passed_losers_mom)}")
    
    if len(passed_momentum) > 0:
        wr_mom = len(passed_winners_mom) / len(passed_momentum) * 100
        print(f"Win Rate: {wr_mom:.1f}%")
        print(f"Total PnL: {passed_pnl_mom:+.2f}%")
    ...
    if len(passed_momentum) > 0:
        print(f"{'RSI 60-85 (momentum zone)':<35} {len(passed_momentum):>8} {len(passed_winners_mom):>10} {len(passed_losers_mom):>10} {wr_mom:>9.1f}% {passed_pnl_mom:>+11.2f}%")
    ...
    for low, high, label in rsi_ranges:
        ...
        if count > 0:
            ...
            tokens = [w['token'] for w in winners if low <= w['rsi'] < high]
            print(f"{label}: {count} winners ({', '.join(tokens)}), total PnL = {total_pnl:+.2f}%")
    
    # Data quality note
    print("\n" + "=" * 80)
    print("DATA QUALITY NOTE")
    print("=" * 80)
    print("RSI computed from 1h candles available BEFORE each signal's created_at timestamp.")
    print("This provides historical RSI at the time of signal detection.")
    print("Note: Signal actually uses 5m candles for RSI computation.")
    print("1h RSI may differ from 5m RSI, but should be directionally similar.")
