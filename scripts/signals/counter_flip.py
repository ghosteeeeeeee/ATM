#!/usr/bin/env python3
"""
MIGRATION: counter_flip_signal.py → signals/counter_flip.py
============================================================
Moved from /root/.hermes/scripts/counter_flip_signal.py to
/root/.hermes/scripts/signals/counter_flip.py as part of the signals
directory re-organization. The signal logic is unchanged; only the
location and the feature flag have been updated (CASCADE_FLIP_ENABLED
→ COUNTER_FLIP_ENABLED).
...
def run() -> int:
    """
    Main entry point for the counter_flip signal.

    Guarded by COUNTER_FLIP_ENABLED — returns 0 immediately if disabled.
    """
    if not COUNTER_FLIP_ENABLED:
        print("[counter-flip] Exiting: COUNTER_FLIP_ENABLED is False")
        return 0

    return scan_counter_flip_signals()
...
if __name__ == '__main__':
    print(f"[counter-flip] COUNTER_FLIP_ENABLED={COUNTER_FLIP_ENABLED}")
    if not COUNTER_FLIP_ENABLED:
        print("[counter-flip] Exiting: COUNTER_FLIP_ENABLED is False")
    else:
