#!/usr/bin/env python3
"""Range Reversion SHORT — Sell at range top.

Mean-reversion signal for flat/ranging markets. SHORT ONLY.

THESIS:
  In COMPRESSED regimes (BB width < 2%), price oscillates between bands.
  Selling at the upper band with overbought RSI captures the rejection back to mid/lower.

  NOTE: SHORT side was DISABLED (2026-09-02) — backtest showed mostly losses.
  SHORT signals bleed in trending-up markets. Re-enable only in confirmed downtrend.

ENTRY CONDITIONS:
  1. Bollinger Band width is narrow (range confirmed, BBWIDTH < 4%)
  2. Price touches upper BB band (within 0.3*ATR)
  3. RSI is overbought (> 65)
  4. Price shows rejection from upper band (current < prev)

Architecture:
  5m candles → BB computation → range detection → RSI filter → rejection confirmation
...
SIGNAL_TYPE = 'range_reversion_short'
SOURCE = 'range-reversion-short-'
SHADOW_MODE = False
...
def _get_5m_candles(token: str, lookback: int = RR_LOOKBACK_5M) -> list:
    ...
    try:
        ...
        _log(f"  [range-reversion-short] candles error {token}: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def detect_range_reversion_short(token: str, candles: list) -> Optional[dict]:
    """Detect SHORT entry: price at upper BB band + overbought RSI + rejection."""
...
def scan_range_reversion_short_signals(prices_dict: dict) -> int:
    """Main entry point — scan all tokens for SHORT range reversion."""
    ...
    for token, data in prices_dict.items():
        ...
        sig = detect_range_reversion_short(token, candles)
        if sig is None:
            continue
        ...
        _log(f"  {tag} SHORT-range-reversion {token:8s} conf={sig['confidence']}% "
        ...
        try:
            ...
            if sid:
                added += 1
                set_cooldown(token, 'SHORT', hours=RR_COOLDOWN_MINUTES / 60.0)
        except Exception as e:
            _log(f"  [range-reversion-short] add_signal error {token}: {e}")
...
def run(prices_dict=None) -> int:
    ...
    return scan_range_reversion_short_signals(prices_dict)
...
if __name__ == '__main__':
    ...
    n = run(prices)
    print(f"[range-reversion-short] Done. {n} signals emitted.")
