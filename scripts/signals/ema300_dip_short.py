...
Only fires when:
  ...
  3. EMA300 slope < 0 (downtrend is real, not flattening)
  4. Price within 0.5% of EMA300 (rally)
  5. RSI > 65 (overbought within downtrend)
  6. Red candle (bounce rejection confirmation)
  7. Price stayed below EMA300 for 20+ consecutive candles (sustained downtrend)

Signal type: ema300_dip_short
Source: ema300-dip-short

Key insight from trade analysis (16 trades, 7W/9L):
  - ALL 9 losers had POSITIVE EMA slope (uptrend disguised as downtrend)
  - ALL 7 winners had NEGATIVE EMA slope (genuine downtrend)
  - ALL 9 losers had 0 consecutive candles below EMA (just crossed above)
  - Winners had 51-205 consecutive candles below EMA (sustained)
"""
...
from hermes_constants import (
    ...
    EMA300_DIP_SHORT_MAX_CROSSINGS,
...
def detect_ema300_dip_short(token, candles, price):
    ...
    Fires SHORT when:
      ...
      - Price stayed below EMA300 for 20+ consecutive candles (sustained downtrend)
    """
    n = len(candles)
    if n < 500:
        return None
    ...
    trend_strength = 100 - (above_count / lookback * 100)
    ...
    # ── Condition 7: Sustained downtrend (consecutive candles below EMA) ──
    # Price must have stayed below EMA300 for N+ consecutive candles.
    # ALL 9 losers had consec=0 (just crossed above EMA).
    # Winners had consec 51-205 (sustained downtrend).
    consec_below = 0
    ...
    if consec_below < EMA300_DIP_SHORT_MIN_CONSEC_BELOW:
        return None
