# Migrated from ../r2_rev_5m_signals.py — see signals/__init__.py registry
#!/usr/bin/env python3
"""
...
def scan_r2_rev_5m_signals(prices_dict: dict) -> int:
    from hermes_constants import R2_REV_ENABLED
    if not R2_REV_ENABLED:
        return 0
    ...
    for token, data in prices_dict.items():
        ...
        from hermes_constants import R2_REV_PLUS_ENABLED, R2_REV_MINUS_ENABLED
        if sig['direction'] == 'LONG' and not R2_REV_PLUS_ENABLED:
            continue
        if sig['direction'] == 'SHORT' and not R2_REV_MINUS_ENABLED:
            continue
...
def run(prices_dict=None):
    ...
    return scan_r2_rev_5m_signals(prices_dict)
