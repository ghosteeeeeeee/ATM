#!/usr/bin/env python3
"""Accel-300 V2 LONG 5m — Strong Trend Momentum Signal (LONG-only, 5-minute timeframe).

5m version of accel_300_v2_long.py. Uses 5m candles for less noise and
more reliable signals. EMA300 on 5m = 25 hours of data.

LONG fires when:
  1. Price above EMA300 with gap >= 1.5%
  2. Gap is accelerating (widening over 10-bar window = 50 min)
  3. Price velocity positive (price moving up over 5 bars = 25 min)
  4. Price persisted above EMA for 3+ bars (15 min)
  5. Linear regression slope positive (20 bars = 100 min)
...
# ── 5m LONG-specific constants ───────────────────────────────────────────────
# All tunables in hermes_constants.py (ACCEL_300_V2_LONG_5M_*)
# These are local fallbacks / internal implementation details
V2_MIN_GAP_ACCEL = 0.15        # min gap acceleration (positive = widening for LONG)
V2_GAP_ACCEL_WINDOW = 10       # bars (50 min on 5m)
V2_VELOCITY_WINDOW = 5         # bars (25 min on 5m)
V2_PERSISTENCE_BARS = 3        # min bars price must stay above EMA (15 min)
V2_VOLUME_LOOKBACK = 30        # bars for average volume
V2_VOLUME_MULT = 1.0           # volume must be >= average
V2_COOLDOWN_BARS = 6           # cooldown between signals per token (30 min)
V2_LOOKBACK_5M = 400           # 5m candles to fetch (33 hours)
V2_SLOPE_WINDOW = 20           # bars (100 min on 5m)
V2_MIN_SLOPE_PCT = 0.0003      # min slope % per bar (lower than 1m — 5m less noisy)
V2_MIN_GAP_PCT = 1.5           # min gap % for confidence bonus
V2_FRESH_CROSS_BARS = 6        # max bars since cross (30 min on 5m)
V2_FRESH_CROSS_MIN_GAP = 0.50  # min gap for fresh cross entry

# Direction-specific gap thresholds (from hermes_constants.py)
from hermes_constants import (
    ACCEL_300_V2_LONG_5M_MIN_GAP, ACCEL_300_V2_LONG_5M_MAX_GAP,
)

PERIOD = 300  # EMA300 period (300 * 5min = 25 hours)
DRY_RUN = '--dry' in sys.argv

SIGNAL_TYPE = 'accel_300_v2_long_5m'
SOURCE = 'accel-300-v2-long-5m'
...
def _get_5m_candles(token: str, lookback: int = V2_LOOKBACK_5M) -> list:
    """Fetch 5m close prices from candles_5m, oldest first."""
    conn = None
    try:
        ...
        if (time.time() - most_recent_ts) > 600:  # 10 min staleness for 5m
            return []

        return [{'timestamp': r[0], 'price': r[1]} for r in rows]

    except Exception as e:
        print(f"  [accel-300-v2-long-5m] candles_5m error for {token}: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
# ═══════════════════════════════════════════════════════════════════════════════
# Detection — LONG-only, 5m timeframe
# ═══════════════════════════════════════════════════════════════════════════════

def detect_accel_300_v2_long_5m(token: str, candles: list) -> Optional[dict]:
    """Detect strong LONG momentum on 5m — gap above EMA300 large AND accelerating.

    LONG fires when:
      1. Price above EMA300 with gap >= 1.5%
      2. Gap is accelerating (widening over 10-bar window)
      3. Price velocity positive (price moving up)
      4. Price persisted above EMA for 3+ bars
    ...
    closes = [float(c['price']) for c in candles]
    ...
    if abs_gap < ACCEL_300_V2_LONG_5M_MIN_GAP or abs_gap > ACCEL_300_V2_LONG_5M_MAX_GAP:
        return None
...
def scan_accel_300_v2_long_5m_signals(prices_dict: dict) -> int:
    """Scan tokens for accel_300_v2_long_5m (strong trend momentum LONG 5m) signals."""
    from hermes_constants import (
        ACCEL_300_V2_LONG_5M_ENABLED,
        LONG_BLACKLIST,
    )
    ...
    for token, data in prices_dict.items():
        ...
        if not candles or len(candles) < PERIOD + 30:
            continue

        sig = detect_accel_300_v2_long_5m(token, candles)
        if sig is None:
            continue
        ...
        if DRY_RUN:
            _log(f"  [DRY] LONG-accel-300-v2-long-5m {token:8s} conf={confidence:.0f}% "
        ...
        current_candles = _get_5m_candles(token, lookback=PERIOD + 10)
        if current_candles and len(current_candles) >= PERIOD:
            current_closes = [float(c['price']) for c in current_candles]
            ...
            if current_ema and current_ema > 0:
                ...
                if abs_current_gap < ACCEL_300_V2_LONG_5M_MIN_GAP or abs_current_gap > ACCEL_300_V2_LONG_5M_MAX_GAP:
                    continue
        ...
        try:
            ...
            if sid:
                ...
                _log(f"  LONG-accel-300-v2-long-5m {token:8s} conf={confidence:.0f}% "
                      f"price={signal_price:.8g} gap={sig['gap_pct']:.3f}% "
                      f"accel={sig['gap_acceleration']:.3f}% "
                      f"gap_vel={sig['gap_velocity']:.3f}% "
                      f"trend_1h={trend_1h} [{SOURCE}]")
        except Exception as e:
            print(f"[accel-300-v2-long-5m] add_signal error for {token}: {e}")
...
if __name__ == '__main__':
    ...
    print(f"[accel-300-v2-long-5m] Testing on {len(prices)} tokens ({mode} mode)...")
    init_db()
    n = scan_accel_300_v2_long_5m_signals(prices)
    print(f"[accel-300-v2-long-5m] Done. {n} signals emitted.")
...
def run(prices_dict=None):
    ...
    return scan_accel_300_v2_long_5m_signals(prices_dict)
