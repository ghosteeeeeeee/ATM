#!/usr/bin/env python3
"""
r2_trend_v2_long.py — R² Trend Confirmation Signal v2 for LONG entries.

Independent copy of r2_trend_long.py with own constants for independent tuning.
...
from hermes_constants import (
    R2_TREND_V2_LONG_ENABLED,
    R2_TREND_V2_LONG_MIN_SLOPE_PCT,
    R2_TREND_V2_LONG_MIN_R2,
    R2_TREND_V2_LONG_MAX_RSI,
    R2_TREND_V2_LONG_MIN_RSI,
    R2_TREND_V2_LONG_MIN_SPEED,
    R2_TREND_V2_LONG_MAX_BB_POS,
    R2_TREND_V2_LONG_BLOCK_STALE,
    R2_TREND_V2_LONG_MAX_ACCEL,
    R2_TREND_V2_LONG_MIN_PRE_MOVE,
    R2_TREND_V2_LONG_MAX_GAP300,
    R2_TREND_V2_LONG_MIN_R2_RISE,
...
# ── Constants ─────────────────────────────────────────────────────────────
R2_WINDOW            = 16
SIGNAL_TYPE          = 'r2_trend_v2_long'
SOURCE_PREFIX        = 'r2v2-long'
LOOKBACK_CANDLES     = 50
MIN_CONFIDENCE       = 50
MAX_CONFIDENCE       = 88
BASE_CONFIDENCE      = 65
...
def detect_r2_long(token, candles, price):
    ...
    Fires LONG when:
      - R² >= R2_TREND_V2_LONG_MIN_R2 (confirmed trend, not chop)
      - Slope > 0 (uptrend)
    ...
    if r2 < R2_TREND_V2_LONG_MIN_R2 or slope_pct <= R2_TREND_V2_LONG_MIN_SLOPE_PCT or closes[-1] <= intercept:
        return None
    ...
    try:
        ...
        if rows_ema and len(rows_ema) >= 300:
            ...
            if gap300 > R2_TREND_V2_LONG_MAX_GAP300:
    ...
    try:
        ...
        if rows_pm and len(rows_pm) >= 15:
            ...
            if pre_move < R2_TREND_V2_LONG_MIN_PRE_MOVE:
    ...
    if len(closes) >= R2_WINDOW + 3:
        ...
        if not (r2_prev < R2_TREND_V2_LONG_MIN_R2 and r2 >= R2_TREND_V2_LONG_MIN_R2):
            # Also allow if R² is rising strongly (even if already above threshold)
            if r2 < R2_TREND_V2_LONG_MIN_R2 or (r2 - r2_prev) < R2_TREND_V2_LONG_MIN_R2_RISE:
                return None
    elif r2 < R2_TREND_V2_LONG_MIN_R2:
        return None
    ...
    for i in range(n - R2_WINDOW, -1, -1):
        ...
        if b_i <= 0 or r2_i < R2_TREND_V2_LONG_MIN_R2:
            break
    ...
    # Confidence scoring
    r2_bonus = min((r2 - R2_TREND_V2_LONG_MIN_R2) / (1.0 - R2_TREND_V2_LONG_MIN_R2) * R2_BONUS_MAX, R2_BONUS_MAX)
    ...
    for token, data in prices.items():
        ...
        try:
            ...
            if _spd_row:
                ...
                if R2_TREND_V2_LONG_BLOCK_STALE and _is_stale:
                    continue
                if _speed is not None and _speed < R2_TREND_V2_LONG_MIN_SPEED:
                    continue
                if _accel is not None and _accel > R2_TREND_V2_LONG_MAX_ACCEL:
                    continue
        except Exception as _e:
            print(f'  [r2_trend_v2_long] WARN: failed to read token_speeds for {token}: {_e}')
        finally:
            if _conn_spd:
                _conn_spd.close()
        ...
        if len(closes_list) >= 15:
            ...
            if avg_loss > 0:
                ...
                rsi = 100.0
            if rsi > R2_TREND_V2_LONG_MAX_RSI:
                continue
        ...
        if len(closes_list) >= 20:
            ...
            if std_20 > 0:
                ...
                if bb_pos > R2_TREND_V2_LONG_MAX_BB_POS:
                    continue
...
if __name__ == '__main__':
    ...
    init_db()
    n = scan_signals()
    print(f'[r2_trend_v2_long] Done. {n} signals emitted.')
