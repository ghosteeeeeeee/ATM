# Migrated from ../volume_hl_signals.py — see signals/__init__.py registry
#!/usr/bin/env python3
"""
...
def scan_token(token: str) -> bool:
    from hermes_constants import VOLUME_HL_ENABLED
    if not VOLUME_HL_ENABLED:
        return []
    """
    ...
    from hermes_constants import VOLUME_HL_PLUS_ENABLED, VOLUME_HL_MINUS_ENABLED
    if direction == 'LONG' and not VOLUME_HL_PLUS_ENABLED:
        return None
    if direction == 'SHORT' and not VOLUME_HL_MINUS_ENABLED:
        return None
...
def scan_volume_hl_signals(prices_dict: dict) -> int:
    """Scan all tokens in prices_dict for volume HL signals. Returns count."""
    from hermes_constants import VOLUME_HL_ENABLED
    if not VOLUME_HL_ENABLED:
        return 0
    count = 0
    for token in prices_dict.keys():
        if scan_token(token):
            count += 1
    return count
...
def run(prices_dict=None):
    ...
    return scan_volume_hl_signals(prices_dict)
