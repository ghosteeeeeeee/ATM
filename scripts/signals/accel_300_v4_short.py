#!/usr/bin/env python3
"""Accel-300 V4 SHORT — Proven Momentum with Regime Filtering.
...
V2 CONDITIONS (kept — these work):
  1. Price below EMA300 with gap >= 2.0% (strong trend established)
  ...
  6. Gap velocity confirms (not narrowing)
  7. Multi-bar gap confirmation
  8. Fresh cross gap check
...
REGIME PERFORMANCE (V2 historical):
  ...
  EXTREME: 52% WR, +$0.36
  NORMAL:  50% WR, +$0.16
  FLAT:    0 trades (V2 naturally avoided, but V4 blocks explicitly)
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# Data fetch — V2 style (proven)
# ═══════════════════════════════════════════════════════════════════════════════

def _get_1m_prices(token: str, lookback: int = ACCEL_300_V4_SHORT_LOOKBACK_1M) -> list:
...
def _check_volume(token: str) -> bool:
    ...
    try:
        ...
        c.execute("""
            ...
            ORDER BY ts ASC
        """, (token.upper(), ACCEL_300_V4_SHORT_VOLUME_LOOKBACK))
        rows = c.fetchall()
        ...
        return volumes[-1] >= avg_vol * ACCEL_300_V4_SHORT_VOLUME_MULT
    except Exception:
...
# ═══════════════════════════════════════════════════════════════════════════════
# Detection — V4 Proven Momentum SHORT
# ═══════════════════════════════════════════════════════════════════════════════

def detect_accel_300_v4_short(token: str, prices: list) -> Optional[dict]:
    """Detect strong SHORT momentum — V2 proven conditions with regime filter.
    ...
    # ── FILTER 1: Gap in valid range (V2 proven: 2.0-6.0%) ─────────────────
    if abs_gap < ACCEL_300_V4_SHORT_MIN_GAP or abs_gap > ACCEL_300_V4_SHORT_MAX_GAP:
        return None

    # ── FILTER 2: Gap acceleration (10-bar window, must be negative for SHORT)
    accel_start = latest_idx - ACCEL_300_V4_SHORT_GAP_ACCEL_WINDOW
    if accel_start < 0:
        return None
    ...
    if gap_acceleration > -ACCEL_300_V4_SHORT_MIN_GAP_ACCEL:
        return None
    ...
    fresh_cross = bars_since_cross <= ACCEL_300_V4_SHORT_FRESH_CROSS_BARS

    # ── FILTER 4: Price velocity (must be negative for SHORT) ───────────────
    if latest_idx < ACCEL_300_V4_SHORT_VELOCITY_WINDOW:
        return None
    price_velocity = closes[latest_idx] - closes[latest_idx - ACCEL_300_V4_SHORT_VELOCITY_WINDOW]
    ...
    min_velocity = abs(closes[latest_idx]) * ACCEL_300_V4_SHORT_MIN_VELOCITY
    ...
    persist_start = latest_idx - ACCEL_300_V4_SHORT_PERSISTENCE_BARS + 1
    if persist_start < 0:
        return None
    ...
    slope_window = min(ACCEL_300_V4_SHORT_SLOPE_WINDOW, len(closes))
    if slope_window >= 2:
        ...
        if denominator > 0 and y_mean != 0:
            ...
            if pct_slope >= -ACCEL_300_V4_SHORT_MIN_SLOPE_PCT:
                return None
    ...
    if fresh_cross and abs_gap < ACCEL_300_V4_SHORT_FRESH_CROSS_MIN_GAP:
...
def scan_accel_300_v4_short_signals(prices_dict: dict) -> int:
    """Scan tokens for accel_300_v4_short (proven momentum SHORT) signals."""
    from hermes_constants import (
    ...
    for token, data in prices_dict.items():
        ...
        sig = detect_accel_300_v4_short(token, prices)
        if sig is None:
            continue
        ...
        try:
            ...
            if _last:
                ...
                if _elapsed < ACCEL_300_V4_SHORT_COOLDOWN_BARS:
        ...
        # ── V4 NEW: FLAT regime block ───────────────────────────────────────
        # FLAT regime = 17% WR for SHORT signals — skip entirely
        try:
            ...
            if not trade_ok:
                continue
            if regime == 'FLAT':
                continue  # FLAT kills SHORT signals
        except Exception:
            pass  # if volatility_gate unavailable, don't block

        # Confidence: V2 proven formula
        gap_bonus = min(20, (abs(sig['gap_pct']) - ACCEL_300_V4_SHORT_MIN_GAP) * 10)
        ...
        if current_ema and current_ema > 0:
            ...
            if abs_current_gap < ACCEL_300_V4_SHORT_MIN_GAP or abs_current_gap > ACCEL_300_V4_SHORT_MAX_GAP:
                continue
        ...
        try:
            ...
            if sid:
                added += 1
                set_cooldown(token, 'SHORT', hours=ACCEL_300_V4_SHORT_COOLDOWN_BARS / 60.0)
