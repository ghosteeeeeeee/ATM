...
def detect(token):
    ...
    # 7. Momentum alignment check (CEO 2026-09-20 — data-flipped)
    # SHORT: rising=65%WR+$0.17, flat=39%WR-$0.63, falling=41%WR-$0.24 (7d)
    # Block SHORT when momentum is flat or falling (anti-trend for SHORT)
    # Block LONG when momentum is rising (anti-trend for LONG)
    if len(closes) >= 6 and closes[-6]:
        vel_5bar = (closes[-1] - closes[-6]) / closes[-6] * 100
        momentum_state = 'rising' if vel_5bar > 0.1 else 'falling' if vel_5bar < -0.1 else 'flat'
        if direction == 'SHORT' and momentum_state in ('flat', 'falling'):
            return None  # momentum against SHORT direction
        if direction == 'LONG' and momentum_state in ('rising', 'flat'):
            return None  # momentum against LONG direction
    else:
        momentum_state = 'flat'
