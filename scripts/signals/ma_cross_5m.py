# Migrated from ../ma_cross_5m.py — see signals/__init__.py registry
#!/usr/bin/env python3
"""
...
def init_tuner_db():
    ...
    os.makedirs(DATA_DIR, exist_ok=True)  # ensure parent dir exists
...
def scan_ma_cross_5m_signals(prices_dict: dict) -> int:
    from hermes_constants import MA_CROSS_5M_ENABLED
    if not MA_CROSS_5M_ENABLED:
        return 0
    """
    ...
    for token, data in prices_dict.items():
        ...
        for direction in ('SHORT', 'LONG'):
            ...
            from hermes_constants import MA_CROSS_5M_PLUS_ENABLED, MA_CROSS_5M_MINUS_ENABLED
            if direction == 'LONG' and not MA_CROSS_5M_PLUS_ENABLED:
                continue
            if direction == 'SHORT' and not MA_CROSS_5M_MINUS_ENABLED:
                continue
...
def run(prices_dict=None):
    """Entry point for signals_runner (signals/__init__._run_signal)."""
    if prices_dict is None:
        from signal_schema import get_all_latest_prices
        prices_dict = get_all_latest_prices()
    return scan_ma_cross_5m_signals(prices_dict)
