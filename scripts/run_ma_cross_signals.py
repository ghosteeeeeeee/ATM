#!/usr/bin/env python3
"""
run_ma_cross_signals.py — Standalone 10/200 EMA Cross Signal Runner.

Run this script to scan all tokens for MA cross signals and write them to the
signals_hermes_runtime.db via signal_schema.add_signal().

Can be run manually:  python3 scripts/run_ma_cross_signals.py
...
def get_latest_prices_from_candles() -> dict:
    ...
    try:
        ...
        print(f"[ma_cross] candles.db price fetch error: {e}")
    return prices
...
def get_open_positions() -> dict:
    ...
    try:
        ...
        print(f"[ma_cross] positions table error: {e}")
    return positions
...
def main():
    print(f"[ma_cross] Starting MA cross scan at {time.strftime('%Y-%m-%d %H:%M:%S')}")

    # 1. Get live prices
    prices = get_latest_prices_from_candles()
    print(f"[ma_cross] Found {len(prices)} tokens with price data")

    # 2. Get open positions (skip if already open)
    open_pos = get_open_positions()
    if open_pos:
        print(f"[ma_cross] {len(open_pos)} open positions: {open_pos}")
    ...
    if skipped:
        print(f"[ma_cross] Skipped {skipped} blacklisted tokens")
    ...
    # 5. Remove cooldown tokens — save eligible list BEFORE removal
    from signal_schema import is_cooldown_active
    eligible_tokens = dict(filtered)  # tokens that will be scanned
    removed_cooldown = 0
    for token in list(filtered.keys()):
        if is_cooldown_active(token, 'LONG') or is_cooldown_active(token, 'SHORT'):
            ...
            removed_cooldown += 1
    if removed_cooldown:
        print(f"[ma_cross] Skipped {removed_cooldown} cooldown tokens")

    print(f"[ma_cross] Scanning {len(filtered)} tokens for MA cross signals...")

    # 6. Run MA cross scanner
    from signal_schema import init_db, record_cooldown_start
    init_db()

    n = scan_ma_cross_signals(filtered)

    # 7. Write cooldowns via signal_schema (uses milliseconds internally).
    # Standalone runner writes ONLY for tokens that actually fired (coin + direction).
    # When called via signal_gen.py, cooldowns are handled by position_manager.
    if n:
        cooldown_count = 0
        for sig in n:  # n is now a list of {'token': str, 'direction': str}
            record_cooldown_start(sig['token'], sig['direction'], MA_COOLDOWN_MINUTES)
            cooldown_count += 1
        print(f"[ma_cross] Cooldowns written: {cooldown_count} signals × {MA_COOLDOWN_MINUTES}min")

    print(f"[ma_cross] Done. {len(n)} signals emitted.")
