# Graph Report - scripts  (2026-08-05)

## Corpus Check
- 275 files · ~450,513 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4353 nodes · 8574 edges · 223 communities (219 shown, 4 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 99 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a0cb4dc8`
...
- _check_counter_trend_trap
- _get_hotset_approval_rate
- detect_ma_fast_cross
- _fetch_binance_candles
- check_atr_tp_sl_hits
- _call_minimax
- backtest
- backtest_fast
- sweep_r2_5m.py
...
1. `log()` - 266 edges
2. `add_signal()` - 150 edges
3. `get_all_latest_prices()` - 91 edges
...
- `run()` --calls--> `log()`  [INFERRED]
  scripts/signals/phase_accel.py → scripts/signals/wyckoff.py
- `scan_accel_300_signals()` --calls--> `is_delisted()`  [INFERRED]
  scripts/accel_300_signals.py → scripts/hyperliquid_exchange.py
- `get_realized_pnl()` --calls--> `wavg_price()`  [INFERRED]
  scripts/hyperliquid_exchange.py → scripts/backfill_orphan_hl_prices.py
- `_wait_for_hl_close()` --indirect_call--> `_wait_for_position_closed()`  [INFERRED]
  scripts/cascade_flip.py → scripts/hl-sync-guardian.py
...
## Communities (223 total, 4 thin omitted)
...
### Community 1 - "price_age_minutes"
Cohesion: 0.03
Nodes (117): detect_accel_300(), _ema_series(), _get_1m_prices(), _log(), Fetch 1m close prices from price_history (signals_hermes.db), oldest first.…, Write to both stdout and signals.log., Detect persistent gap above EMA(300) with growing gap. Fire when ALL of these…, Scan tokens for accel_300 signals. All guards (blacklists, open positions,… (+109 more)

### Community 2 - "signal_gen.py"
Cohesion: 0.03
Nodes (101): is_delisted(), Return True if token is delisted/halted on Hyperliquid (no new positions)., _get_candles_1m(), Fetch 1m close prices from price_history (signals_hermes.db), oldest first.…, Scan pre-filtered tokens for support/resistance signals and write to DB. All…, scan_rs_signals(), check_long_trend_filter(), check_short_trend_filter() (+93 more)

### Community 3 - "hyperliquid_exchange.py"
Cohesion: 0.04
Nodes (89): _place_or_replace_tp(), Place a new TP order if none exists, or replace the existing TP order. Returns…, _asset_id(), build_order(), cancel_bulk_orders(), cancel_sl(), cancel_tp(), clean_all_tpsl_orders() (+81 more)

### Community 4 - "signal_schema.py"
Cohesion: 0.04
Nodes (81): _ema(), Compute EMA(period) from a list of prices (oldest first). Returns the most…, Add RSI as standalone signal — filtered to only fire when trend aligns., _run_rsi_signals_for_confluence(), compute_all_indicators(), compute_macd(), compute_rsi(), compute_zscore() (+73 more)

### Community 5 - "decider_run.py"
Cohesion: 0.05
Nodes (62): detect_incomplete_run(), On startup, check for a pipeline run that was interrupted mid-trade. Returns…, _apply_inversion(), _check_hotset_cooldown(), context_gate(), _ctx_load_cache(), _ctx_save_cache(), execute_trade() (+54 more)

### Community 6 - "__init__.py"
Cohesion: 0.05
Nodes (53): # NOTE: 'vel-hermes' bare sentinel removed — vel-hermes+/vel-hermes- now…, # NOTE: hzscore+,hzscore- merge artifacts are now IMPOSSIBLE because, # NOTE: do NOT use this for PnL calculations — use, # NOTE: signals/rs.py had hardcoded values that diverged from this file., # NOTE: price_history is close-only (open=high=low=close per row), so swing, # NOTE: Lines 373-384 removed 2026-05-06 — were duplicate with inconsistent…, # NOTE: inv-accel-300- is DISABLED (INVERSE_ACCEL_300_MINUS_ENABLED=False ) but…, # NOTE: momentum+/momentum- had NO Layer 2 kill-switch in signal_schema.py… (+45 more)

### Community 7 - "self_close_watcher.py"
Cohesion: 0.05
Nodes (58): get_hype_all_mids_batched(), Update current_price for all open trades using Hyperliquid prices, Get all mids from shared HL cache (written by price_collector)., update_trade_prices(), cache_age(), cache_fresh(), get_atr(), ATR cache — persistent file + in-memory ATR cache with 300s TTL. Survives… (+50 more)
...
### Community 9 - "add_signal"
Cohesion: 0.05
Nodes (54): detect_exhaustion(), _ema(), main(), Scan all tokens (or single token) and emit exhaustion signals. Args: conf_min:…, Compute EMA30 over a list of closing prices., Detect exhaustion reversal signal for a token. exhaustion SHORT: prior…, scan(), detect_gap300_5m() (+46 more)

### Community 10 - "log"
Cohesion: 0.06
Nodes (54): run(), run(), log(), cancel_orders_for_coin(), close_position_on_hl(), get_hl_open_orders(), get_hl_positions(), get_paper_trades() (+46 more)
...
Cohesion: 0.05
Nodes (54): calc_ema_series(), ComboStats, find_crosses(), get_candles(), get_tokens(), main(), Fetch 1m candles for a token (oldest first)., EMA series — None before warmup, float after. (+46 more)
...
### Community 15 - "get_db"
Cohesion: 0.08
Nodes (44): get_db(), init_db(), Get database connection with WAL mode., Create all tables if they don't exist., generate_report(), main(), Run one complete cycle: scan, monitor, report., Run as daemon with periodic cycles. (+36 more)
...
Cohesion: 0.07
Nodes (41): add_tag(), add_trade(), backfill_embeddings(), call_ollama(), close_trade(), _close_trade_impl(), extract_and_store(), get_db_connection() (+33 more)
...
Cohesion: 0.07
Nodes (20): main(), HebbianEngine, main(), Ensure consistent ordering for symmetric storage., Record that concept_a and concept_b fired together. Increments synapse weight.…, Learn all pairs from a set of concepts that fired together. Creates C(n,2)…, Decrement synapse weight between two concepts (loss learning). Creates the…, Hebbian write-back from a closed trade. Won (pnl_pct > 0) → strengthen all… (+12 more)

### Community 23 - "_get_conn"
Cohesion: 0.08
Nodes (38): clear_hotset(), count_signals(), main(), purge_signals(), run_ai_decider(), cleanup_stale_approved(), clear_cooldown_entry(), get_approved_signals() (+30 more)

### Community 24 - "position_manager.py"
Cohesion: 0.08
Nodes (38): _clean_expired(), clear_loss_streak(), enforce_max_positions(), get_loss_cooldown_remaining(), get_loss_streak(), _get_macd_1h_state(), get_opposite_direction_cooldown_hours(), is_loss_cooldown_active() (+30 more)
...
Cohesion: 0.10
Nodes (32): close_brain(), close_paper(), get_exit_price(), main(), Get the most recent fill price for a coin from HL trade history. NOTE:…, Remove coin from paper open, append to closed with reason=manual_close., Close the open brain trade for coin with reason=manual_close., Record loss cooldown based on PnL. Wins do NOT trigger cooldown. (+24 more)
...
### Community 32 - "rs.py"
Cohesion: 0.09
Nodes (31): _atr(), _atr_pct(), _bounce_confirmation(), _build_level_touches(), _cluster_levels(), _compute_confidence(), detect_rs_signal(), _find_swing_highs_lows() (+23 more)

### Community 33 - "unified_scanner.py"
Cohesion: 0.10
Nodes (30): add_all_signals(), get_cached_indicators(), get_cached_prices(), get_fear(), get_gateio_rsi(), get_gateio_signals(), get_hyperliquid_tokens(), get_open_trades() (+22 more)
...
Cohesion: 0.10
Nodes (29): cascade_flip(), _close_paper_position(), _get_db_connection(), insert_post_flip_trade(), mark_token_flipped(), Update flip_counts entry for a token that just underwent a cascade flip. Adds…, Synchronously insert a DB entry for a position opened via cascade flip. Sets…, _load_flip_counts() (+21 more)
...
### Community 41 - "scripts/ma_cross_5m.py"
Cohesion: 0.12
...
### Community 42 - "refresh_current_prices"
Cohesion: 0.10
Nodes (22): main(), force_atr_update(), apply_pnl_ground_truth(), compute_hl_pnl_pct(), compute_live_pnl(), pnl_sanity_check(), Compute pnl_pct from HL's unrealized_pnl and position_value. Used when we have…, Apply HL ground truth at close time. When hype_pnl_usdt is available (HL fills… (+14 more)
...
### Community 54 - "check_and_manage_positions"
Cohesion: 0.12
Nodes (22): _load_hotset_failures(), Load per-direction failure counts. {TOKEN: {'LONG': {'count': N, 'last': ts},…, READ-ONLY hot-set enforcer (defunct approval logic — signal_compactor.py is the…, _run_hot_set(), adjust_stop_loss(), check_and_manage_positions(), check_stale_position(), _collect_atr_updates() (+14 more)
...
Cohesion: 0.12
Nodes (22): _atr(), _count_bounces_with_rejection(), _detect_breakout(), _detect_fakeout(), detect_tl_break(), _detect_trendline(), _get_candles_5m(), _linear_regression_with_r2() (+14 more)
...
Cohesion: 0.12
Nodes (21): get_hype_meta_batched(), Get meta from shared HL cache (written by price_collector)., cache_age(), cache_fresh(), fetch_and_cache(), fetch_and_cache_positions(), get_cached_positions(), get_meta() (+13 more)
...
Cohesion: 0.17
Nodes (19): atr_check(), atr_sl_hit(), atr_tp_hit(), _base(), guardian_cycle(), log_event(), loss_cooldown_set(), _now() (+11 more)
...
Cohesion: 0.14
Nodes (15): grep_file(), Return list of (line_num, line) matching pattern., get_fast_signals(), get_registered_signals(), get_slow_signals(), Resolve 'enabled' to bool: if string, look up in hermes_constants; otherwise…, Return only the signals where enabled=True and run is not None., Fast signals — run every minute. (+7 more)
...
### Community 79 - "compute_mtf_macd_alignment"
Cohesion: 0.15
Nodes (18): cascade_entry_signal(), compute_mtf_macd_alignment(), _detect_cascade(), Detect cascade entry condition: smaller TF flips before larger TF confirms.…, Detect cascade entry timing and generate entry/exit signals. Key insight:…, Compute multi-timeframe MACD alignment using the FULL macd_rules state machine.…, _check_cascade_direction_flip(), _check_macd_rules_flip() (+10 more)

### Community 80 - "profit_monster.py"
Cohesion: 0.17
Nodes (18): close_position(), filter_by_pnl(), get_all_open_positions(), is_position_on_hl(), is_token_being_closed_by_guardian(), load_config(), Check if token still has an open position on HL., Check if guardian closing markers include this token. (+10 more)
...
Cohesion: 0.17
Nodes (15): IntEnum, compute_macd_state(), CrossoverFreshness, ema(), get_macd_bullish_score(), get_macd_entry_signal(), get_macd_exit_signal(), load_token_macd_params() (+7 more)
...
### Community 110 - "close_paper_position"
Cohesion: 0.14
Nodes (14): Record outcome when a trade closes. Updates both the JSON file (legacy) and the…, record_ab_trade_closed(), Log an A/B test outcome to W&B (offline) for visual comparison. Also appends a…, record_ab_outcome(), cancel_all_open_orders(), Cancel ALL open orders (trigger AND non-trigger) for a coin in ONE API call.…, _bridge_signal_history_to_patterns(), close_paper_position() (+6 more)
...
Cohesion: 0.22
Nodes (12): _get_candles_1m(), Fetch 1m close prices from price_history (signals_hermes.db), oldest first.…, Scan pre-filtered tokens for 8/50 MA cross SHORT signals and write to DB. All…, scan_ma_fast_signals(), get_latest_prices_from_candles(), get_open_positions(), is_blacklisted(), main() (+4 more)
...
### Community 134 - "run_better_coder.py"
Cohesion: 0.24
...
### Community 139 - "hermes_log.py"
Cohesion: 0.22
Nodes (8): increment_pipeline_cycle(), Increment and persist the pipeline cycle counter. Called once per pipeline run…, main(), Run a step in the background so the pipeline is not blocked. Used for slow…, # NOTE: --live is NOT passed to step scripts., run(), run_bg(), Centralized secret loader — reads from .secrets.local in project root. All…

### Community 140 - "counter_flip_signal.py"
Cohesion: 0.25
...
### Community 150 - "_warmup_volume_cache"
Cohesion: 0.20
...
### Community 153 - "get_category_multipliers"
Cohesion: 0.25
...
### Community 161 - "_analyze_loss_direction"
Cohesion: 0.25
Nodes (8): _analyze_loss_direction(), is_wrong_side_risky(), _load_wrong_side(), Load wrong-side learning data., Save wrong-side learning data., Post-mortem on a losing trade. Check: did price move AGAINST us first (counter-…, Pre-trade check: should we be more careful entering this token+direction?…, _save_wrong_side()

### Community 162 - "_dr_atr"
Cohesion: 0.25
...
### Community 175 - "macd_accel_signals.py"
Cohesion: 0.33
Nodes (6): _get_1m_closes(), Scan for MACD acceleration signals across all tokens. Args: prices_dict:…, Fetch 1m close prices from candles.db (candles_1m table). Returns: list of…, Return SMA series (oldest first), None for indices < period-1., scan_macd_accel_signals(), _sma()
...
### Community 177 - "run_rs_signals.py"
Cohesion: 0.43
...
### Community 181 - "get_signal_streak"
Cohesion: 0.33
Nodes (6): _get_signal_streak(), Get cached signal streak, refreshed every 5 minutes (per-key TTL)., _ensure_signal_outcomes_table(), get_signal_streak(), Create signal_outcomes table if it doesn't exist. Add trade_id column if…, Get recent win/loss streak and streak-adjusted weight for a signal. Returns:…
...
### Community 191 - "check_cascade_flip"
Cohesion: 0.40
...
### Community 200 - "_check_counter_trend_trap"
Cohesion: 0.50
Nodes (4): _check_counter_trend_trap(), _get_token_zscore(), Get z-score for a token from signal_gen's zscore computation. Returns 0.0 if…, SPEED FEATURE: Counter-trend trap detection. PENALTY not block: strong signals…

### Community 201 - "_get_hotset_approval_rate"
Cohesion: 0.50
Nodes (4): _get_hotset_approval_rate(), _increment_hotset_approval_rate(), Return (count, window_start_ts). Resets if window expired (>60s)., Save updated approval rate counter.

### Community 202 - "detect_ma_fast_cross"
Cohesion: 0.50
Nodes (4): detect_ma_fast_cross(), _ema_series(), Compute EMA series — returns EMA value at each index (oldest first). Returns a…, Detect 8/50 EMA crossover on 1m candles — SHORT only. Args: token: token symbol…

### Community 203 - "_fetch_binance_candles"
Cohesion: 0.50
Nodes (4): _fetch_binance_candles(), Fetch OHLCV candles for any interval via signal_schema (unified price source).…, fetch_binance_candles(), Fetch OHLCV candles from Binance public API (no auth required). Writes results…

### Community 204 - "check_atr_tp_sl_hits"
Cohesion: 0.50
Nodes (4): check_atr_tp_sl_hits(), _is_closing_marker_active(), Check if guardian is currently closing this token — skip ATR check if so., Check every open position for ATR TP/SL hit. Returns list of dicts with…
...
- **Why does `log()` connect `log` to `hl-sync-guardian.py`, `price_age_minutes`, `signal_gen.py`, `decider_run.py`, `hermes-brain-sync.py`, `run_better_coder.py`, `candle_predictor.py`, `hermes_log.py`, `hh_hl.py`, `hyperliquid-trader.py`, `pump_hunter.py`, `study_winning_combos.py`, `_get_conn`, `position_manager.py`, `scan_for_signals`, `run_guppy_signals.py`, `get_open_hype_positions_curl`, `signal_compactor.py`, `_analyze_loss_direction`, `unified_scanner.py`, `ai_decider.py`, `breakout_engine.py`, `refresh_current_prices`, `ab_optimizer.py`, `get_signal_streak`, `check_and_manage_positions`, `sync_open_trades.py`, `4h_regime_scanner.py`, `_acquire_lock_with_heartbeat`, `check_cascade_flip`, `context-compactor.py`, `signal_importer.py`, `15m_regime_scanner.py`, `log_error`, `bug_hunter.py`, `profit_monster.py`, `candle_tuner.py`, `ab_learner.py`, `away_detector.py`, `sync_kanban_tasks.py`, `trading-checklist.py`, `close_paper_position`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `add_signal()` connect `add_signal` to `price_age_minutes`, `signal_gen.py`, `signal_schema.py`, `__init__.py`, `ma_cross.py`, `r2_rev.py`, `counter_flip_signal.py`, `hh_hl.py`, `r2_rev_5m_signals.py`, `wyckoff.py`, `pattern_scanner.py`, `_get_conn`, `position_manager.py`, `zscore_momentum.py`, `exhaustion.py`, `rs.py`, `unified_scanner.py`, `breakout_engine.py`, `signals/ma_cross_5m.py`, `scripts/ma_cross_5m.py`, `oc_signal_importer.py`, `macd_accel_signals.py`, `monte_carlo_gate`, `ema20_50.py`, `get_directional_vol`, `check_and_manage_positions`, `ema20_50_signals.py`, `tl_break.py`, `ma300_candle_confirm_signals.py`, `signal_importer.py`, `gap_300.py`, `ema9_sma20_signals.py`, `compute_mtf_macd_alignment`, `hh_hl_signals.py`, `ma_cross_signals.py`, `fast_momentum.py`, `ema_angle.py`, `mtf_macd.py`, `macd_accel.py`, `atr_compression_signals.py`, `is_cooldown_active`, `record_cooldown_start`, `phase_accel.py`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `get_open_positions()` connect `price_age_minutes` to `signal_gen.py`, `signal_schema.py`, `__init__.py`, `signals/ma_cross_5m.py`, `scripts/ma_cross_5m.py`, `refresh_current_prices`, `log`, `gap_300.py`, `ema9_sma20_signals.py`, `macd_accel_signals.py`, `ema20_50.py`, `run_rs_signals.py`, `wyckoff.py`, `macd_accel.py`, `check_and_manage_positions`, `ema20_50_signals.py`, `position_manager.py`, `fast_momentum.py`?**
...
- **Should `price_age_minutes` be split into smaller, more focused modules?**
  _Cohesion score 0.030880905511811024 - nodes in this community are weakly interconnected._
