...
def get_hot_tokens(min_cycles: int = 1) -> set:
    """
    Return tokens that are currently in the hot-set.
    Hot-set = tokens with hot_cycle_count >= min_cycles in signals DB.
    Only mirrors positions that are in the hot-set.
    """
    try:
        ...
        cur.execute("""
            ...
            FROM signals
            WHERE hot_cycle_count >= ?
        """, (min_cycles,))
        tokens = {r[0] for r in cur.fetchall()}
        conn.close()
        return tokens
    except Exception as e:
        log(f"Failed to get hot tokens from signals DB: {e}", "WARN")
...
def sync_opens(brain_positions, hype_positions):
    ...
    return opened, skipped, delisted, not_hot
...
def main():
    ...
    if not args.closes_only:
        ...
        opened, skipped, delisted, not_hot = sync_opens(brain_pos, hype_pos)
        log(f"Opens: {skipped} already on HL | {delisted} delisted | {not_hot} not-hot | {opened - skipped} to mirror", "INFO")
