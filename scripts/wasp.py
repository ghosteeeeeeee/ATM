...
def check_signals():
    ...
    # Check for duplicate recent signals (same token+dir+signal_type within 5 min)
    # Note: add_signal() dedups by token+direction+signal_type, so WASP should too.
    # Grouping by token+direction only would flag different signal_types as duplicates
    # when they're intentionally separate (RSI vs MACD etc.).
    dups = sqlite(RUNTIME_DB, """
        SELECT token, direction, signal_type, COUNT(*) as cnt, MAX(confidence) as max_conf
        FROM signals
    ...
    if dups:
        bug("WARNING", "signals", "Rapid-fire duplicate signals",
            ", ".join(f"{r['token']}({r['signal_type']},{r['cnt']}x)" for r in dups[:5]))
