...
def execute_trade(token, direction, price, confidence, source,
    ...
    try:
        ...
        try:
            ...
            log(f"  ⚠️ [SL-ZONE-SIZE] Error for {token}: {e}", 'WARN')
        ...
        log(f"  ⚠️ [SL-ZONE-SIZE] Error: {e}", 'WARN')
            ...
            if trade_match:
                ...
                if 'hl_copy_trader' in (source or '') and signal_metadata:
                    try:
                        ...
                        import traceback
                        log(f'  [COPY-PERF] Failed: {_cp_err}\n{traceback.format_exc()[:300]}')
...
def run(dry_run=False):
    ...
    for i, sig in enumerate(scored):
        ...
        if _is_accel_v2 or _is_accel_v2_long or _is_accel_v2_long_5m or _is_accel_v3_short or _is_accel_v3_long or _is_accel_v4_short:
            ...
            try:
                if _is_accel_v2_long_5m:
                    ...
                    fresh_prices = _get_1m_prices(token)
                    if not fresh_prices:
                        log(f'  🚫 [ACCEL-V2-SHORT-STALE] {token} {direction} blocked: no fresh price data for staleness check')
                        if sig_id:
                            mark_signal_executed(token, direction, 'SKIPPED', signal_id=sig_id)
                        skipped += 1
                        continue
        ...
        try:
            from hermes_constants import CHASE_FILTER_ENABLED, CHASE_ZSCORE_MAX, CHASE_GAP_MAX_PCT
        except ImportError:
            CHASE_FILTER_ENABLED = False
        if CHASE_FILTER_ENABLED and direction.upper() == 'LONG':
            _sig_z = sig.get('z_score')
            if _sig_z is None:
                _sig_z = _ctx_gate_get_zscore(token)
            _is_chase = False
            _chase_reason = ''
            if _sig_z is not None and float(_sig_z) > CHASE_ZSCORE_MAX:
                _is_chase = True
                _chase_reason = f'z={_sig_z}>{CHASE_ZSCORE_MAX}'
            elif _gap_at_entry is not None and _gap_at_entry > CHASE_GAP_MAX_PCT:
                _is_chase = True
