...
print(f"\nTables in candles.db: {tables}")

# Try to get BTC candles
# Common table names: btc_1m, candles_1m, etc.
try:
    # Check for BTC table
    cursor.execute("SELECT * FROM btc_1m LIMIT 1")
    print("Found btc_1m table")
    table_name = "btc_1m"
except:
    try:
        cursor.execute("SELECT * FROM candles_1m WHERE symbol='BTC' LIMIT 1")
        print("Found candles_1m table with BTC data")
        table_name = "candles_1m"
    except:
        print("Could not find BTC candle table")
        table_name = None

if table_name:
    # Get BTC price at trade open times
    ...
    for trade in pump_chain_plus:
        ...
        if btc_price:
            btc_prices_plus.append((btc_price, pnl, coin))
            if pnl > 0:
                wins_plus += 1
            else:
                losses_plus += 1
    ...
    if btc_prices_plus:
        ...
        # For now, let's use absolute price and look for relative moves
        ...
        for price, pnl, coin in btc_prices_plus:
            momentum = (price - avg_btc) / avg_btc * 100
            if momentum > threshold_pct:
                above_threshold.append((price, pnl, coin, momentum))
            else:
                below_threshold.append((price, pnl, coin, momentum))
        ...
        if above_threshold:
            wins_above = sum(1 for _, pnl, _, _ in above_threshold if pnl > 0)
            losses_above = sum(1 for _, pnl, _, _ in above_threshold if pnl <= 0)
            ...
            print(f"\n  Individual trades:")
            for price, pnl, coin, mom in above_threshold:
                print(f"    {coin}: BTC ${price:,.2f} (+{mom:.2f}%), PnL: ${pnl:+.2f}")
        
        print(f"\nTrades where BTC <= +{threshold_pct}%: {len(below_threshold)}")
        if below_threshold:
            wins_below = sum(1 for _, pnl, _, _ in below_threshold if pnl > 0)
            losses_below = sum(1 for _, pnl, _, _ in below_threshold if pnl <= 0)
    ...
    for trade in pump_chain_minus:
        ...
        btc_price = get_btc_price_at(open_time)
        if btc_price:
            btc_prices_minus.append((btc_price, pnl, coin))
            if pnl > 0:
                wins_minus += 1
            else:
                losses_minus += 1
    ...
    if btc_prices_minus:
        ...
        for price, pnl, coin in btc_prices_minus:
            momentum = (price - avg_btc_minus) / avg_btc_minus * 100
            if momentum < threshold_pct:
                below_threshold_minus.append((price, pnl, coin, momentum))
            else:
                above_threshold_minus.append((price, pnl, coin, momentum))
        ...
        if below_threshold_minus:
            wins_below_minus = sum(1 for _, pnl, _, _ in below_threshold_minus if pnl > 0)
            losses_below_minus = sum(1 for _, pnl, _, _ in below_threshold_minus if pnl <= 0)
            ...
            # Show individual trades
            print(f"\n  Individual trades:")
            for price, pnl, coin, mom in below_threshold_minus:
                print(f"    {coin}: BTC ${price:,.2f} ({mom:.2f}%), PnL: ${pnl:+.2f}")
        
        print(f"\nTrades where BTC >= -{abs(threshold_pct)}%: {len(above_threshold_minus)}")
        if above_threshold_minus:
            wins_above_minus = sum(1 for _, pnl, _, _ in above_threshold_minus if pnl > 0)
            losses_above_minus = sum(1 for _, pnl, _, _ in above_threshold_minus if pnl <= 0)
    ...
    if btc_prices_plus and above_threshold:
        ...
        saved_plus = sum(abs(pnl) for _, pnl, _, _ in above_threshold if pnl <= 0)
    ...
    if btc_prices_minus and below_threshold_minus:
        ...
        saved_minus = sum(abs(pnl) for _, pnl, _, _ in below_threshold_minus if pnl <= 0)
