...
def _get_momentum_velocity(token: str, candles: int = 3) -> float:
    ...
    try:
        ...
        # Calculate velocity as average % change over recent candles
        velocities = []
...
def manage_ride_it_exit(token: str, direction: str, current_price: float,
    ...
    if not entry_price or not current_price or not open_time_str:
        return {'action': 'HOLD'}
    ...
    if hold_seconds < RIDE_IT_PHASE1_TO_PHASE2_TIME:
        ...
        phase1_sl = entry_price - sl_distance if direction == 'LONG' else entry_price + sl_distance
        
        # Only update SL if it's tighter (higher for LONG, lower for SHORT)
        if direction == 'LONG' and phase1_sl > current_sl:
        ...
        elif direction == 'SHORT' and phase1_sl < current_sl:
            _persist_sl(trade_id, phase1_sl)
            log(f"  [RIDE-IT-P1] {token} {direction}: SL → ${phase1_sl:.6f} ({sl_distance/entry_price*100:.1f}%)")
            return {'action': 'TRAIL_SL', 'new_sl': phase1_sl}
    ...
    # Check momentum exit (from pump exit)
    neg_candles = _get_neg_candle_count(token, RIDE_IT_MOMENTUM_CANDLES)
    if neg_candles >= RIDE_IT_MOMENTUM_CANDLES:
        # Check if we're in profit — only exit on momentum if we have something to protect
        if profit_pct > 0.01:  # >1% profit
            reason = f"ride_it_momentum: neg_candles={neg_candles}, profit={profit_pct*100:+.1f}%"
            log(f"  [RIDE-IT] {token} {direction}: {reason}")
            return {'action': 'EXIT', 'reason': reason}
