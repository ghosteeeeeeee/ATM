#!/usr/bin/env python3
"""
_aggregate_5m.py — Standalone 5m candle aggregator.

Reads from signals_hermes.db price_history (always fresh 1m bars),
aggregates into candles_hermes.db candles_5m.

Self-healing: fills any missed closed windows, writes developing candle
for the open window when >= 2 bars are available.
...
Timer: hermes-5m-candle.timer (every 5 min)
"""
...
def migrate_is_closed():
    """Add is_closed=1 to all existing candles_5m rows that lack the column."""
...
def aggregate_5m():
    ...
    if not clock_row or not clock_row[0]:
        print("[5m] No price_history data")
        ph_conn.close()
        candle_conn.close()
        return
    now = clock_row[0]
    ...
    print(f"[5m] filled={filled} closed windows, dev={dev_written} tokens, clock={time.strftime('%H:%M:%S', time.localtime(now))}, last_closed={last_closed} ({last_dt})")
