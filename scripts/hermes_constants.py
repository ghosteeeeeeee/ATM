...
SHORT_BLACKLIST = {
    ...
    'GRIFFAIN','BRETT','XLM','SNX','NIL','IP','TRB','ETHFI','EIGEN','S','VVV','SUI','LAYER','BERA','DYM','MAVIA','MEME','INIT','SOPH','XAI','ZEC','GAS','BLAST','MELANIA','ZETA','SPX','ARK','RUNE','AR',
...
CHASE_GAP_MAX_PCT = 2.0              # brain_auditor 2026-09-25: tightened 3.0→2.0. Gap>2.0% = 8T/14d 25%WR -$1.10. Only 2 winners ($0.23). Blocks BABY(1.687), ACE(2.56), ADA(1.95) chases.
...
CHASE_GAP_MAX_PCT = 2.0           # brain_auditor 2026-09-25: tightened 3.0→2.0. Gap>2.0% = 8T/14d 25%WR -$1.10.
...
PULLBACK_ENTRY_SHORT_DEAD_HOURS = [0, 1, 3, 4, 6, 8, 10, 11, 13, 20, 22]  # auto_1hr 2026-09-25 — added 22 (5T 20%WR -$0.60/14d, worst remaining losing hour). Net dead hours: +$5.74/14d = +$2.87/7d.
...
ACCEL_300_ENABLED        = True   # RE-ENABLED 2026-09-11 — master switch for SHORT. NORMAL/EXTREME winning, HIGH blocked at 0.3x.
...
COILED_SPRING_COIL_ATR_PCT_MAX  = 0.45    # ATR% must be below this (volatility compressed) — tightened from 0.55
...
COILED_SPRING_MIN_CONDITIONS    = 5       # minimum conditions (of 6) that must pass
...
EMA300_DIP_SHORT_ENABLED = False      # DISABLED 2026-09-09 — 24T, 41.7% WR, NO winning regime (HIGH 42%, NORMAL 42%). Kill correct.
...
GRIND_TREND_MINUS_ENABLED = False          # KILLED signal_reporter 2026-09-19 — 20% WR, -$0.38 (24h), no winning regime. All-time: 5 trades, 20% WR.
...
ACCEL_300_V3_LONG_REEXPAND_MIN = 0.20   # min gap re-expansion (raised from 0.10: filters weak bounces like ICP at 0.06%)
...
ACCEL_300_V3_LONG_COOLDOWN_BARS = 15    # cooldown between signals (15 min — matched SHORT 2026-09-08)
...
ACCEL_300_V3_SHORT_RSI_MIN     = 25      # min RSI — below this = oversold, bounce likely (catches JUP 22, ENS 17, NEAR 20)
...
# ── accel-300-v4 SHORT params (proven momentum + regime filter) ───────────────
...
ACCEL_300_V4_SHORT_GAP_ACCEL_WINDOW = 10  # bars for gap acceleration (V2 proven)
ACCEL_300_V4_SHORT_VELOCITY_WINDOW = 5    # bars for price velocity (V2 proven)
ACCEL_300_V4_SHORT_MIN_VELOCITY = 0.0005  # min velocity as fraction of price (0.05%) — V2 proven
ACCEL_300_V4_SHORT_PERSISTENCE_BARS = 3   # min bars below EMA (V2 proven)
ACCEL_300_V4_SHORT_SLOPE_WINDOW = 20      # bars for linear regression slope (V2 proven)
ACCEL_300_V4_SHORT_MIN_SLOPE_PCT = 0.0005 # min slope % per bar (V2 proven)
ACCEL_300_V4_SHORT_FRESH_CROSS_BARS = 8   # max bars since cross (V2 proven)
ACCEL_300_V4_SHORT_FRESH_CROSS_MIN_GAP = 0.20 # min gap for fresh cross (V2 proven)
ACCEL_300_V4_SHORT_VOLUME_LOOKBACK = 30   # bars for average volume (V2 proven)
ACCEL_300_V4_SHORT_VOLUME_MULT = 1.0      # volume must be >= average (V2 proven: any volume OK)
ACCEL_300_V4_SHORT_COOLDOWN_BARS = 15     # cooldown between signals (V2 proven)
ACCEL_300_V4_SHORT_LOOKBACK_1M = 700      # 1m prices to fetch (V2 proven)
...
INVERSE_ACCEL_300_V2_MAX_GAP_PCT = 8.0    # don't fire if gap too extreme (structural, not reversion)
INVERSE_ACCEL_300_V2_REVERSION_BARS = 3   # bars of gap narrowing to confirm
INVERSE_ACCEL_300_V2_REVERSION_THRESHOLD = 0.40  # min gap narrowing % (backtested optimal)
INVERSE_ACCEL_300_V2_GAP_VELOCITY_WINDOW = 5  # bars to measure gap velocity
INVERSE_ACCEL_300_V2_STABILIZATION_WINDOW = 15  # bars to check for turning point
INVERSE_ACCEL_300_V2_STABILIZATION_TOLERANCE = 0.01  # 1% — must be near recent high
INVERSE_ACCEL_300_V2_VOLUME_LOOKBACK = 30  # bars to compute average volume
INVERSE_ACCEL_300_V2_VOLUME_MULT = 1.2    # volume must be 1.2x average (selling pressure)
INVERSE_ACCEL_300_V2_VELOCITY_WINDOW = 5  # bars to measure price velocity
INVERSE_ACCEL_300_V2_TREND_FILTER_PCT = 2.5  # max 1h move — rally IS the setup for mean reversion
INVERSE_ACCEL_300_V2_COOLDOWN_BARS = 10   # cooldown between signals per token (10 min)
INVERSE_ACCEL_300_V2_LOOKBACK_1M = 700    # 1m prices to fetch per token
INVERSE_ACCEL_300_V2_MAX_SLIPPAGE_PCT = 0.30  # max slippage from detection price
INVERSE_ACCEL_300_V2_SLIPPAGE_BARS = 3    # bars to check for slippage
...
BB_BOUNCE_V2_LONG_ENABLED = True   # CEO RE-ENABLED 2026-09-22 — signal_reporter killed Sep 11 (4T/24h 25%WR) but30d shows73T 74%WR +$2.08. Short-term variance, not systemic. NEVER_REENABLE=False.
...
CONTINUATION_WINDOW_SEC = 2700                # seconds after close to scan (45 min, was 30 min — catches delayed re-entries)
...
ATR_SPIKE_ENABLED              = False   # CEO KILLED 2026-08-28 — 7T/7d 28.6% WR -$0.15, ALL ATR_SL exits, avg loss -1.09%. NEVER_REENABLE.
...
SIGNAL_CONFLUENCE_MIN_FAVORABLE_MOVE = 0.02  # 2% — price must move this much in favorable direction to confirm
SIGNAL_CONFLUENCE_MIN_COMPOUND         = 3      # min unique sources to fire (3+ only — 2 sources too noisy)
...
RR_STRUCTURAL_MINUS_ENABLED   = False  # KILLED 2026-09-14 CEO: 7T/7d 42.9%WR -$0.42, all NEUTRAL, below 50% WR threshold
...
PULLBACK_SUPPORT_PROXIMITY_PCT   = 0.2     # block SHORT when entry < 0.2% above recent 5m low (at support = bounce risk)
PULLBACK_SUPPORT_LOOKBACK        = 20      # candles to look back for support/resistance levels
