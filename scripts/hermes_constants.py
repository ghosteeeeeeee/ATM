...
ATR_SL_MIN             = 0.012   # 1.2% floor — widened from 1.0% (2026-08-21). 1.0% was binding constraint on nearly every ATR_SL loss — normal 1% pullbacks = instant SL hit with 5x. 1.2% gives breathing room while staying tight. Backtested: 75% WR vs 71% at 1.0%, 0 winners harmed.
...
ATR_SL_MIN_INIT    = 0.012  # 1.2% — widened from 1.0% (2026-08-21). MUST match ATR_SL_MIN
...
CONF_FILTER_MAX = 85                    # block if confidence >= this value (lowered from 89 — 90+ tier has 48.7% WR, worst tier)
...
ACCEL_300_REGIME_SLOPE_PCT = 0.01    # RE-RAISED 2026-08-22 by T — was 0.0005%, then 0.005%. 0.01% filters real uptrends, not noise.
