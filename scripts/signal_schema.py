...
def add_signal(token, direction, signal_type, source, confidence, value=None, price=None,
    ...
    try:
        ...
        for _comp in _components:
            ...
            if _comp in ('accel-300-v3-long+', 'accel-300-v3-long-'):
                try:
                    from hermes_constants import ACCEL_300_V3_LONG_ENABLED
                    if not ACCEL_300_V3_LONG_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" ACCEL_300_V3_LONG_ENABLED=False', flush=True)
                        return None
                except ImportError:
                    pass
            if _comp == 'accel-300-v3-long':
                try:
                    from hermes_constants import ACCEL_300_V3_LONG_ENABLED
                    if not ACCEL_300_V3_LONG_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" ACCEL_300_V3_LONG_ENABLED=False', flush=True)
            ...
            if _comp == 'bb_bounce-':
                try:
                    ...
                    pass
            # bb_bounce_long (V2 solo variant with velocity filter)
            if _comp in ('bb_bounce_long', 'bb-bounce-long+'):
                try:
                    from hermes_constants import BB_BOUNCE_LONG_ENABLED
                    if not BB_BOUNCE_LONG_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" BB_BOUNCE_LONG_ENABLED=False', flush=True)
                        return None
                except ImportError:
                    pass
            ...
            if _comp == 'zscore-rising+':
                try:
                    from hermes_constants import ZSCORE_RISING_PLUS_ENABLED
                    if not ZSCORE_RISING_PLUS_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" ZSCORE_RISING_PLUS_ENABLED=False', flush=True)
                        return None
                except ImportError:
                    pass
            if _comp == 'zscore-rising-':
                try:
                    from hermes_constants import ZSCORE_RISING_MINUS_ENABLED
                    if not ZSCORE_RISING_MINUS_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" ZSCORE_RISING_MINUS_ENABLED=False', flush=True)
                        return None
                except ImportError:
                    pass
            if _comp == 'zscore-rising':
                try:
                    from hermes_constants import ZSCORE_RISING_ENABLED
                    if not ZSCORE_RISING_ENABLED:
                        print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" ZSCORE_RISING_ENABLED=False', flush=True)
                        return None
                except ImportError:
                    pass
    ...
    try:
        ...
        if _comp == 'wall-st-cycle+':
            try:
                from hermes_constants import WALL_ST_CYCLE_PLUS_ENABLED
                if not WALL_ST_CYCLE_PLUS_ENABLED:
                    print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" WALL_ST_CYCLE_PLUS_ENABLED=False', flush=True)
                    return None
            except ImportError:
                pass
        if _comp == 'wall-st-cycle-':
            try:
                from hermes_constants import WALL_ST_CYCLE_MINUS_ENABLED
                if not WALL_ST_CYCLE_MINUS_ENABLED:
                    print(f'  DEBUG add_signal BLOCKED: {token} {direction} source="{source}" WALL_ST_CYCLE_MINUS_ENABLED=False', flush=True)
                    return None
            except ImportError:
                pass
        c.execute('''
...
def is_component_disabled(component: str) -> bool:
    ...
    try:
        from hermes_constants import (
            ...
            CONTINUUM_TREND_ENABLED, CONTINUUM_TREND_ALIGN_ENABLED, CONTINUUM_TREND_REVERSAL_ENABLED,
    ...
    # continuation
    if c == 'continuation': return not CONTINUATION_ENABLED
    if c == 'continuation+': return not CONTINUATION_PLUS_ENABLED
    if c == 'continuation-': return not CONTINUATION_MINUS_ENABLED
    # neutral-sniper
    if c == 'neutral-sniper': return not NEUTRAL_SNIPER_ENABLED
    if c == 'neutral-sniper+': return not NEUTRAL_SNIPER_PLUS_ENABLED
    if c == 'neutral-sniper-': return not NEUTRAL_SNIPER_MINUS_ENABLED
    # mover
    if c == 'mover': return not MOVER_ENABLED
    if c == 'mover+': return not MOVER_PLUS_ENABLED
    if c == 'mover-': return not MOVER_MINUS_ENABLED
    # zscore-pump
