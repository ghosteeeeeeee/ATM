...
from hermes_constants import SHORT_BLACKLIST, LONG_BLACKLIST, SIGNAL_SOURCE_BLACKLIST, SPEED_HOTSET_BONUS, SPEED_HOTSET_THRESHOLD, CONFLUENCE_REQUIRED, CONFLUENCE_NEUTRAL_RELAX, ACCEL_300_STANDALONE_BYPASS_ENABLED, ACCEL_300_STANDALONE_BYPASS_CONFIDENCE, ACCEL_300_REGIME_SLOPE_PCT, TOKEN_WR_THRESHOLD, TOKEN_WR_MIN_SAMPLE, STANDALONE_BYPASS_SIGNALS, FAVORITES, FAVORITES_MULT, FAVORITES_RESIDENCY_DECAY, PENALTY_TOKENS, PENALTY_MULT, SHORT_NEUTRAL_BLOCK_ENABLED, LONG_NEUTRAL_BLOCK_ENABLED, LOSERS, LOSERS_MULT, AMPLITUDE_COMPACTOR_MULT, ACCEL_300_V3_SHORT_EXTREME_BLOCK, ACCEL_300_V3_SHORT_FLAT_BLOCK, ACCEL_300_V3_LONG_EXTREME_BLOCK, ACCEL_300_V3_LONG_FLAT_BLOCK, ACCEL_300_MINUS_FLAT_BLOCK, BTC_CHOP_GATE_ENABLED, BTC_CHOP_GATE_THRESHOLD, PUMP_FLOW_SHORT_15M_THRESHOLD, PM_TRAIL_BYPASS_SIGNALS
try:
...
SIGNAL_SOURCE_WEIGHTS = {
    ...
    # accel_300_v4_short: proven momentum SHORT + regime filter (2026-09-11)
    ('accel_300_v4_short', 'accel-300-v4-short-'):  1.0,   # V4 SHORT: V2 proven + FLAT block
    ...
    ('coin_tracker_hot_long', 'ct-hot+'): 1.0,   # Re-enabled 2026-08-21 — LONG only, composite 60+
    ...
    # ema300_breakthrough — price crashes through EMA300 (15m timeframe)
...
def _score_signal(token, direction, conf, source, signal_type,
    ...
    # Use rolling amplitude cache (30d 1h candles) instead of static classification
    _amp_data = _get_amp_cache(token) if _get_amp_cache else None
    amp_class = _amp_data['amp_class'] if _amp_data else 'MED_AMP'
            ...
            if direction.upper() == 'SHORT' and not _is_reversion:
                try:
                    ...
                    if rows_slope and len(rows_slope) >= 20:
                        ...
                        # Regime-aware: relax slope filter in SHORT_BIAS (short_mult > 1.0)
                        slope_threshold = ACCEL_300_REGIME_SLOPE_PCT
                        try:
                            _conn_reg = sqlite3.connect(STATIC_DB, timeout=3)
                            _reg_row = _conn_reg.execute("SELECT short_mult FROM regime_log ORDER BY timestamp DESC LIMIT 1").fetchone()
                            _conn_reg.close()
