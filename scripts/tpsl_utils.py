...
# New trade (peak≈entry): uses INIT floor (wider for TP, tighter for SL).
    ...
    if _phantom_dbg and (token in ('LINK', 'OG', 'AAVE') or (entry_f > 0 and new_sl > 0 and abs(new_sl - entry_f) / entry_f < 0.0015)):
        print(f"  [TPSL-DEBUG] {token} {direction}: BEFORE MIN GUARD new_sl={new_sl:.6f} entry_f={entry_f:.6f} "
              f"current={current_price:.6f} highest={highest_price:.6f} ref={ref_price:.6f} "
              f"is_new={is_new_trade} eff_sl={eff_sl_pct*100:.3f}% current_sl={current_sl:.6f}")
    if entry_f > 0:
        if direction == 'LONG' and highest_price > 0:
    ...
    if _phantom_dbg and (token in ('LINK', 'OG', 'AAVE') or (entry_f > 0 and new_sl > 0 and abs(new_sl - entry_f) / entry_f < 0.0015)):
        print(f"  [TPSL-DEBUG] {token} {direction}: AFTER MIN GUARD new_sl={new_sl:.6f} "
              f"entry_f={entry_f:.6f} eff_sl={eff_sl_pct*100:.3f}% current_sl={current_sl:.6f}")
