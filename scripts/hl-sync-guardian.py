...
"""
import sys, time, json, subprocess, argparse, os, re, fcntl, math
sys.path.insert(0, '/root/.hermes/scripts')
from _secrets import BRAIN_DB_DICT
from hermes_file_lock import FileLock
from pnl_utils import compute_close_pnl, compute_live_pnl
...
def _retry_phantom_close_fills():
    ...
    try:
        ...
        for trade_row in phantom_trades:
            ...
            # BUG-FIX (2026-06-10): same root cause as line 2523 fix.
            ...
            if close_fills:
                ...
                # BUG-FIX (2026-06-10): PnL formula must be direction-aware.
                # LONG: profit when exit > entry. SHORT: profit when exit < entry.
                if direction == 'SHORT':
                    pnl_pct = ((entry_px - wavg_exit) / entry_px) * 100
                else:
...
def _poll_close_fills_once(token: str):
    ...
    # It MISSES all LONG closes (dir="Close Long" has side='A').
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            if token in hl_pos:
                ...
                # FIX (2026-05-20): Always update prices and pnl_pct, even when
                # unrealized_pnl == 0 (breakeven positions). The old guard skipped
                # the entire update when pnl was zero, leaving current_price stale.
                # We use unrealized_pnl for pnl_usdt (HL's authoritative USDT value)
                # and compute_live_pnl for pnl_pct (unleveraged, entry-based %).
                pnl_usdt = round(unrealized_pnl, 4)  # HL unrealized PnL in USDT
...
def _check_hard_stops(prices: dict):
    ...
    try:
        ...
        for trade_id, token, direction, entry_px, sl, tp, lev, amt, paper, atr_managed, signal in rows:
            ...
            # Import SIGNAL_EXIT_CONFIG once for both branches
            ...
            if hit_reason:
                ...
                if hl_pos and token.upper() not in hl_pos:
                    ...
                    # FIX: Use _close_paper_trade_db instead of direct UPDATE.
                    # The old UPDATE skipped pnl_usdt (stayed 0.00) and wrote
                    # unleveraged pnl_pct (missing * leverage). _close_paper_trade_db
                    # properly computes both and records loss cooldown.
                    _CLOSED_HL_COINS.add(token.upper())
                    try:
                        _close_paper_trade_db(trade_id, token, cur_price,
                                               f'position_manager_{hit_reason}')
                    except Exception as e:
                        log(f'  [HARD-{hit_reason.upper()}] DB close failed for '
                ...
                if closed_ok:
                    ...
                    # FIX: Use _close_paper_trade_db instead of direct UPDATE.
                    # The old UPDATE had two bugs:
                    #   1. pnl_usdt was never written (stayed 0.00 from open state)
                    #   2. pnl_pct was unleveraged (missing * leverage multiplier)
                    # _close_paper_trade_db properly computes leveraged pnl_pct,
                    # fetches HL fills for ground truth pnl_usdt, records loss
                    # cooldown, and clears reconciled state.
                    _CLOSED_HL_COINS.add(token.upper())
                    _close_paper_trade_db(trade_id, token, exit_price, hit_reason)
                    log(f'  [HARD-{hit_reason.upper()}] {token} closed at {exit_price:.6f} '
                        f'[hl_fill={hl_fill_price}]', 'PASS')
                else:
                    # BUG-FIX (2026-04-20): When HL close fails after 2 attempts,
                    # force-close the paper trade to prevent it from becoming an orphan.
        ...
        for row in live_missing:
            ...
            try:
                ...
                if hl_exit_px > 0:
                    ...
                    # FIX: Use _close_paper_trade_db for proper PnL computation.
                    # The old direct UPDATE skipped pnl_pct and pnl_usdt entirely.
                    try:
                        _CLOSED_HL_COINS.add(token.upper())
                        _close_paper_trade_db(trade_id, token, hl_exit_px, close_reason)
...
def _close_paper_trade_db(trade_id, token, exit_price, reason):
    ...
    try:
        ...
        # Calculate PnL using centralized pnl_utils
...
def _close_orphan_paper_trade_by_id(trade_id, token, direction, entry_px, lev, reason, amount_usdt_override=None):
    """
    ...
    amount_usdt_override: optional float — if provided, use this instead of
    DB lookup. Allows guardian to pass actual HL notional (sz × entry_px)
    computed at orphan-close time before DB INSERT creates the record.
    """
    if DRY:
    ...
    if amount_usdt_override is not None:
        ...
        log(f'  [_close_orphan_paper_trade_by_id] PnL TIER-2: using HL_notional_override=${amount_usdt:.2f} '
            f'(trade_id={trade_id}, reason={reason})', 'INFO')
    elif conn_lookup:
        try:
...
def sync():
    ...
    #
    # BUG-FIX (2026-05-20): Must run AFTER hl_pos is fetched (not before), otherwise
    # when HL is rate-limited and returns empty {}, ALL markers are incorrectly cleared
    # as "stale" before the orphan close ever runs. This caused the closing marker bug
    # where ada/mon/strk/tia markers kept reappearing every cycle.
    # Move to after Step 1 (HL fetch) — see line 3561.
