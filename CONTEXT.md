...
> Full system architecture: `/root/.hermes/ATM/ATM-Architecture.md` — read this for complete pipeline, DB, and service details.

## Key Files for Quick Access
- `/root/.hermes/mcp/hermes-coding-mcp/server.py` — MCP server with Hebbian tools (hebbian_recall, hebbian_learn, hebbian_stats)
- `/root/.hermes/scripts/hebbian_engine.py` — Hebbian associative memory engine (standalone, NOT auto-injected into prompts)
- `/root/.hermes/scripts/decider_run.py`
- `/root/.hermes/scripts/signal_compactor.py` — main signal decision maker (replaces ai_decider.py)
- `/root/.hermes/scripts/position_manager.py`
- `/root/.hermes/scripts/signal_schema.py`
- `/root/.hermes/scripts/ai_decider.py` — defunct/deprecated/obsolete, ignore
- `/var/www/hermes/data/hotset.json`
- `/var/www/hermes/data/trades.json`
- `/root/.hermes/scripts/hl-sync-guardian.py` — the guardian
- `/root/.hermes/data/signals_hermes.db` — current 1min price data for all HL coins
- `/root/.hermes/data/candles.db` — candle data (5min, 15min, 1h, 4h; no 1min candles)

## System Status
> ⚠️ DO NOT trust status blocks in this file — they go stale. Always verify live state:
> - Pipeline log: `tail -100 /root/.hermes/logs/pipeline.log`
> - Hot-set: `cat /var/www/hermes/data/hotset.json | jq '.timestamp'`
> - Live trading: `cat /var/www/hermes/data/hype_live_trading.json`
> - Open positions: `cat /var/www/hermes/data/trades.json | jq '.open | length'`
>
> **Path source:** All paths via `from paths import *` — see `paths.py` for canonical definitions.
...
## Critical Flags
- hype_live_trading.json: ON — live trading kill switch
- Guardian: running (hl-sync-guardian.service)

## Signal DBs
- Hermes: /root/.hermes/data/signals_hermes_runtime.db

## Known Issues (see brain/trading.md for live status)


## Signal Sources (signals_hermes_runtime.db)
| Source | Type | Logic |
|--------|------|-------|
| `hzscore+/hzscore-` | mtf_zscore | Mean-reversion: z<0→LONG, z>0→SHORT |
| `hwave+/hwave-` | hwave | Wave-riding: z near 0 + velocity confirmed → ride the wave |
| `vel-hermes+/vel-hermes-` | velocity | Z-score momentum: velocity direction only |
| `hmacd++/hmacd--` | mtf_macd | MTF MACD crossover |
| `pattern_scanner` | pattern_micro_flag | Pattern-based |
| `pct-hermes+/pct-hermes-` | percentile_rank | Mean-reversion, elevated/suppressed price |
...
| `hermes-signal-compactor.timer` | 1 min | Hot-set compaction (merge + staleness aging) |
...
## ATM_SNAP
# ATM ARCHITECTURE SNAPSHOT (auto-generated
