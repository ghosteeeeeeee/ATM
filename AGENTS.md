...
- **Signal detection vs execution timing** — Signals are detected at one time but executed later. The v2 filters (BB width, RSI, bounce, momentum) check conditions at detection time. If conditions change between detection and execution, trades can pass filters that should block them. Example: CFX trade on 2026-09-06 17:16 — RSI was 44.6 at detection (passes RSI_MIN=35), but dropped to 31.28 by execution. The v2 filters don't re-validate at execution time. **To prevent future occurrences: After committing signal changes, restart the pipeline to ensure the new code is loaded.**
- **⚠️ STALENESS vs TRADE-LENGTH GAP — DO NOT CONFUSE THESE** — This is a recurring misunderstanding. **Staleness = TIME elapsed between signal creation and trade OPEN** (when the position was entered). **Trade CLOSE time = when the position was exited** (SL hit, TP hit, etc). **Trade-length gap = price/EMA gap at execution time** (e.g., "gap was -0.16% when trade was placed"). These are completely different things. The gap at execution time is the MARKET CONDITION when the trade opens — it is NOT a measure of how stale the signal is. When analyzing losses, always check: (1) What is the actual TRADE OPEN time? (2) How much TIME passed between signal creation and trade open? (3) What was the gap at trade open? These are separate questions with separate answers. **SYRUP lesson:** 14:14:19 was trade CLOSE (SL hit), not execution. Actual open was 12:48:31. Don't confuse close time with open time. **BANANA SHORT lesson:** Entry was at RSI=10.53 (extreme oversold). This is NOT a staleness issue or execution timing issue — the entry CONDITIONS were wrong. Shorting oversold = catching falling knife in reverse. Always check RSI/extremes at entry, not just R:R.
