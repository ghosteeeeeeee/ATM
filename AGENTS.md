...
- **⚠️ STALENESS vs TRADE-LENGTH GAP — DO NOT CONFUSE THESE** — This is a recurring misunderstanding. **Staleness = TIME elapsed between signal creation and trade execution** (e.g., "signal created 12.7 hours before execution"). **Trade-length gap = price/EMA gap at execution time** (e.g., "gap was -0.16% when trade was placed"). These are completely different things. The gap at execution time is the MARKET CONDITION when the trade opens — it is NOT a measure of how stale the signal is. When analyzing losses, always check: (1) How much TIME passed between detection and execution? (2) What was the gap at execution time? These are separate questions with separate answers.
